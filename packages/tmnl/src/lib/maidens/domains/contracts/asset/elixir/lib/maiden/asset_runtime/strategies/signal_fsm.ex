defmodule Maiden.AssetRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for asset transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"asset.runtime.strategy.tick", {:strategy_tick}, 90},
      {"asset.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           asset_id: fetch(params, :asset_id) || agent.state[:asset_id],
           kind: fetch(params, :kind) || agent.state[:kind],
           from: from,
           to: to,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :asset_runtime_transition)]
          }
        ]
    else
      directives
