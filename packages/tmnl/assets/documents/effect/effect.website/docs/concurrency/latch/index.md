...
:::::::::: {.sl-container .astro-2wc2n45z style="--borderTop: initial;--pageTitleDisplay: block;"}
:::::: sl-markdown-content
A Latch is a synchronization tool that works like a gate, letting fibers
wait until the latch is opened before they continue. The latch can be
either open or closed:

- When closed, fibers that reach the latch wait until it opens.
- When open, fibers pass through immediately.

Once opened, a latch typically stays open, although you can close it
again if needed

Imagine an application that processes requests only after completing an
initial setup (like loading configuration data or establishing a
database connection). You can create a latch in a closed state while the
setup is happening. Any incoming requests, represented as fibers, would
wait at the latch until it opens. Once the setup is finished, you call
`latch.open`{dir="auto"} so the requests can proceed.
...
aria-hidden="true"}](index.html#the-latch-interface){.anchor-link
aria-labelledby="the-latch-interface"}
:::

A `Latch`{dir="auto"} includes several operations that let you control
and observe its state:

  Operation                Description
  ------------------------ ----------------------------------------------------------------------------------------------------------
  `whenOpen`{dir="auto"}   Runs a given effect only if the latch is open, otherwise, waits until it opens.
  `open`{dir="auto"}       Opens the latch so that any waiting fibers can proceed.
  `close`{dir="auto"}      Closes the latch, causing fibers to wait when they reach this latch in the future.
  `await`{dir="auto"}      Suspends the current fiber until the latch is opened. If the latch is already open, returns immediately.
  `release`{dir="auto"}    Allows waiting fibers to continue without permanently opening the latch.

::: {.autolink-heading-container .level-h2 tabindex="-1"}
## Creating a Latch

[[![](data:image/svg+xml;base64,)]{.anchor-icon
aria-hidden="true"}](index.html#creating-a-latch){.anchor-link
aria-labelledby="creating-a-latch"}
:::

Use the `Effect.makeLatch`{dir="auto"} function to create a latch in an
open or closed state by passing a boolean. The default is
`false`{dir="auto"}, which means it starts closed.

**Example** (Creating and Using a Latch)

In this example, the latch starts closed. A fiber logs "open sesame"
only when the latch is open. After waiting for one second, the latch is
opened, releasing the fiber:
...
::: code
[// A generator function that demonstrates latch
...
::: code
[ ]{.indent}[// Create a latch, starting in the closed
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const latch: Effect.Latch</code></pre>
</figure>
:::
::::

[latch]{style="--0:#005CC5;--1:#79B8FF"}[
]{style="--0:#24292E;--1:#E1E4E8"}[=]{style="--0:#BF3441;--1:#F97583"}[
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const makeLatch: (open?: boolean | undefined) =&gt; Effect.Effect&lt;Effect.Latch, never, never&gt;</code></pre>
</figure>
:::
...
const program = Effect.gen(function*() {  // Create a latch, starting in the closed state  const latch = yield* Effect.makeLatch(false)
  // Fork a fiber that logs &quot;open sesame&quot; when the latch is opened  const fiber = yield* Console.log(&quot;open sesame&quot;).pipe(    latch.whenOpen,    Effect.fork  )
  yield* Effect.sleep(&quot;1 second&quot;)
  // Open the latch  yield* latch.open  yield* fiber.await})
Effect.runFork(program)// Output: open sesame (after 1 second)</code></pre>
</figure>
:::
::::
...
::: {.twoslash-popup-docs .twoslash-popup-docs-tags}
[\@since]{.twoslash-popup-docs-tag-name} ―
[3.8.0]{.twoslash-popup-docs-tag-value}
:::
:::::::

[makeLatch]{style="--0:#6F42C1;--1:#B392F0"}[()]{style="--0:#24292E;--1:#E1E4E8"}
:::::::::::::
::::::::::::::::
...
::: code
[ ]{.indent}[// Fork a fiber that logs \"open sesame\" only when the
latch is open]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const fiber: RuntimeFiber&lt;void, never&gt;</code></pre>
</figure>
:::
::::
...
[log]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[\"open
sesame\"]{style="--0:#032F62;--1:#9ECBFF"}[).]{style="--0:#24292E;--1:#E1E4E8"}[[]{.twoslash-hover}]{.twoslash
style="--0:#6F42C1;--1:#B392F0"}
...
[latch]{style="--0:#24292E;--1:#E1E4E8"}[.]{style="--0:#24292E;--1:#E1E4E8"}[[]{.twoslash-hover}]{.twoslash
style="--0:#24292E;--1:#E1E4E8"}
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>Latch.whenOpen: &lt;A, E, R&gt;(self: Effect.Effect&lt;A, E, R&gt;) =&gt; Effect.Effect&lt;A, E, R&gt;</code></pre>
</figure>
:::
...
This function ensures that the provided effect executes only if the
latch is open. If the latch is closed, the fiber will wait until it
opens.
:::
:::::

[whenOpen]{style="--0:#24292E;--1:#E1E4E8"}[,
]{style="--0:#24292E;--1:#E1E4E8"}[// Waits for the latch to
open]{style="--0:#616972;--1:#99A0A6"}
::::::::
:::::::::::
...
]{style="--0:#24292E;--1:#E1E4E8"}[// Fork the effect into a new
...
::: code
[ ]{.indent}[// Wait for 1 second]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: code
[ ]{.indent}[// Open the latch, releasing the
fiber]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>Latch.open: Effect.Effect&lt;void, never, never&gt;</code></pre>
</figure>
:::
...
::: code
[ ]{.indent}[// Wait for the forked fiber to
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>Fiber&lt;void, never&gt;.await: Effect.Effect&lt;Exit&lt;void, never&gt;, never, never&gt;</code></pre>
</figure>
:::
...
[[![](data:image/svg+xml;base64,)]{.anchor-icon
aria-hidden="true"}](index.html#latch-vs-semaphore){.anchor-link
aria-labelledby="latch-vs-semaphore"}
:::

A latch is good when you have a one-time event or condition that
determines whether fibers can proceed. For example, you might use a
latch to block all fibers until a setup step is finished, and then open
the latch so everyone can continue.

A [semaphore](../semaphore/index.html) with one lock (often called a
binary semaphore or a mutex) is usually for mutual exclusion: it ensures
that only one fiber at a time accesses a shared resource or section of
code. Once a fiber acquires the lock, no other fiber can enter the
protected area until the lock is released.

In short:

- Use a **latch** if you're gating a set of fibers on a specific event
  ("Wait here until this becomes true").
- Use a **semaphore (with one lock)** if you need to ensure only one
  fiber at a time is in a critical section or using a shared resource.
...
.astro-4rgy7crp} Edit
page](https://github.com/Effect-TS/website/edit/main/content/src/content/docs/docs/concurrency/latch.mdx){.sl-flex
...
]{.astro-u5aomj4k}](../semaphore/index.html){.astro-u5aomj4k rel="prev"}
...
]{.astro-u5aomj4k}](../../stream/introduction/index.html){.astro-u5aomj4k
rel="next"}
:::
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
