...
import { RlmStore, RlmStoreLive, type CatalogEntry, type StoredObject, type QueryFilter } from "./service.ts"
import { SearchIndex, SearchIndexLive } from "./search.ts"
import { DomainRegistry, DomainRegistryLive } from "./domains.ts"
import { QueryBuilder } from "./builders.ts"
import { PutBuilder } from "./builders.ts"
import { validateDomainConfig, type DomainConfig } from "./schemas.ts"
 ...
 *
 * Usage (bun, future):
 *   import { sqliteBunLayer } from "./store/sqlite-bun.ts"
 *   const api = createStoreApi(sqliteBunLayer({ filename: dbPath }))
 */
export function createStoreApi(sqlLayer: Layer.Layer<SqlClient>): StoreApi {
  // Service layers — DomainRegistry depends on RlmStore
  const ServiceLayers = Layer.mergeAll(RlmStoreLive, SearchIndexLive, DomainRegistryLive)
  ...
  return {
    ...
    putNow: (collection, prefix, data, tags) =>
      withStore((s) => s.putNow(collection, prefix, data, tags ? { tags } : undefined)),
    ...
    delete: (collection, key) =>
      withStore((s) => s.del(collection, key)),
