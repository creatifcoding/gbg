...
Workbench set the chrome this package ships. Space and blank sockets stay from that extract. Color and type now resolve to TMNL Vantablack. `Table` is the basic TanStack tier. `Grid` wraps `@tmnl/datagrid` for spreadsheet work. Both paint with VANTA.
