...
import { steer, renderAnnotations } from './steer.js'
import { decideLayout, compositeColumns, codePanelLines, codeBlockLines } from './layout.js'
import { isPrimitive, type Primitive } from './primitives/types.js'
import { tryRenderPrimitive } from './primitives/index.js'

// Re-export rendering utilities for downstream consumers
export { highlightCode, getLanguageFromPath, keyHint, Text, type Theme }
export { gridLines } from './grid.js'
export { steer, renderAnnotations } from './steer.js'
...
export function renderCall(
  ...
  let text = theme.fg('toolTitle', theme.bold('cm'))
  ...
  return {
    render(width: number): string[] {
      ...
      try {
        ...
        cachedLines = [
          theme.fg('toolTitle', theme.bold('cm')) + ' ' + theme.fg('error', '✗ render error'),
...
function renderPartial(code: string, theme: Theme): string[] {
  const lines = [theme.fg('toolTitle', theme.bold('cm')) + ' ' + theme.fg('warning', 'evaluating...')]
  if (code) {
...
function renderHeader(parsed: unknown, isStructured: boolean, theme: Theme, primTag?: string, expanded?: boolean): string {
  ...
  const modeHint = expanded
    ...
    : ''
  return theme.fg('toolTitle', theme.bold('cm')) + ' ' + theme.fg('success', '✓')
    + (typeInfo ? ' ' + typeInfo : '')
    + modeHint
}

function renderError(code: string, rawOutput: string, expanded: boolean, width: number, theme: Theme): string[] {
  const lines = [theme.fg('toolTitle', theme.bold('cm')) + ' ' + theme.fg('error', '✗ error')]
