defmodule Maiden.PlantRuntime.Sensors.TransitionSensor do
  @moduledoc """
  Sensor ingress adapter for plant transition events.
  ...
  use Jido.Sensor,
    name: "plant_transition_sensor",
    description: "Converts plant transition events into Jido transition signals",
    schema: Zoi.object(
      %{
        source: Zoi.string() |> Zoi.default("/sensor/plant-transition"),
  ...
  @impl true
  def handle_event({:plant_transition, payload}, state) when is_map(payload) do
  ...
  def handle_event(payload, state) when is_map(payload) do
    handle_event({:plant_transition, payload}, state)
  end

  def handle_event(_event, state), do: {:ok, state}

  defp signal_type(payload), do: "plant.transition." <> payload["to"]

  defp normalize_payload(payload) do
    %{}
    |> put_if_present(payload, "plant_id", :plant_id)
  ...
  defp to_action_params(payload) do
    %{}
    |> put_action_if_present(payload, :plant_id)
  ...
  defp rejection_payload(payload, reason, trace_id) do
    %{
      plant_id: Map.get(payload, "plant_id"),
  ...
  defp attempted_signal_type(payload) do
    case Map.get(payload, "to") do
      to when is_binary(to) and to != "" -> "plant.transition." <> to
      _ -> "plant.transition.unknown"
    end
  end
