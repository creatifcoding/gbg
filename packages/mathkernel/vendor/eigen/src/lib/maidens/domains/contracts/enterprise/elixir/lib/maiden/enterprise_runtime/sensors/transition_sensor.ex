defmodule Maiden.EnterpriseRuntime.Sensors.TransitionSensor do
  @moduledoc """
  Sensor ingress adapter for enterprise transition events.
  ...
  use Jido.Sensor,
    name: "enterprise_transition_sensor",
    description: "Converts enterprise transition events into Jido transition signals",
    schema: Zoi.object(
      %{
        source: Zoi.string() |> Zoi.default("/sensor/enterprise-transition"),
  ...
  @impl true
  def handle_event({:enterprise_transition, payload}, state) when is_map(payload) do
    ...
    case Agent.preflight_transition(normalized) do
      ...
      {:error, reason} = error ->
        if state.config.emit_rejections do
          rejection_signal =
            Jido.Signal.new!(
              "enterprise.transition.rejected",
  ...
  def handle_event(payload, state) when is_map(payload) do
    handle_event({:enterprise_transition, payload}, state)
  end

  def handle_event(_event, state), do: {:ok, state}

  defp signal_type(payload), do: "enterprise.transition." <> Map.get(payload, "to")

  defp normalize_payload(payload) do
    %{}
    |> put_if_present(payload, "enterprise_id", :enterprise_id)
  ...
  defp to_action_params(payload) do
    %{}
    |> put_action_if_present(payload, :enterprise_id)
  ...
  defp rejection_payload(payload, reason, trace_id) do
    %{
      enterprise_id: Map.get(payload, "enterprise_id"),
  ...
  defp attempted_signal_type(payload) do
    case Map.get(payload, "to") do
      to when is_binary(to) and to != "" -> "enterprise.transition." <> to
      _ -> "enterprise.transition.unknown"
    end
  end
