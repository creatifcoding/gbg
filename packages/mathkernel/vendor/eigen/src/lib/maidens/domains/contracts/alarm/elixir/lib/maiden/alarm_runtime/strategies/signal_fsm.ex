defmodule Maiden.AlarmRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for Alarm transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"alarm.runtime.strategy.tick", {:strategy_tick}, 90},
      {"alarm.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           alarm_id: fetch(params, :alarm_id) || agent.state[:alarm_id],
           from: from,
           to: to,
           at: at,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :alarm_runtime_transition)]
          }
        ]
    else
      directives
