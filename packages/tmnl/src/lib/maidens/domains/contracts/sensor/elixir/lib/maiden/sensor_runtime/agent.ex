defmodule Maiden.SensorRuntime.Agent do
  @moduledoc """
  Jido-facing sensor runtime agent.

  ISA-95 alignment:
  - Sensor is the control-module lane under machine/equipment hierarchy.
  """

  alias Maiden.SensorRuntime.Actions.{ObserveRejectedTransition, SetSensorStatus}
  alias Maiden.SensorRuntime.Boundaries
  alias Maiden.SensorRuntime.Directives.{EnqueueTransitionJob, PersistTransition}
  ...
  use Jido.Agent,
    name: "sensor_runtime_agent",
    description: "Sensor lifecycle runtime agent with signal-first strategy",
    schema: [
    ...
    signal_routes: [
      {"sensor.transition.active", SetSensorStatus},
      {"sensor.transition.calibrating", SetSensorStatus},
      {"sensor.transition.needs_calibration", SetSensorStatus},
      {"sensor.transition.faulted", SetSensorStatus},
      {"sensor.transition.offline", SetSensorStatus},
      {"sensor.transition.decommissioned", SetSensorStatus},
      {"sensor.transition.rejected", ObserveRejectedTransition}
    ],
    strategy:
      {SignalFsm,
       # Strategy FSM models instruction execution flow; canonical domain
       # transitions are enforced by Maiden.SensorRuntime.FSM.
       initial_state: "idle",
       transitions: %{
         "idle" => ["processing"],
       ...
       bridge_jobs?: true,
       job_queue: :sensor_runtime_transition}

  @transition_actions [SetSensorStatus]
  ...
  def preflight_agent_state(payload, opts \\ []),
    do: SensorValidator.agent_state_validate(payload, opts)
