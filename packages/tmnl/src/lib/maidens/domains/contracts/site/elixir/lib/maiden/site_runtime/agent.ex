defmodule Maiden.SiteRuntime.Agent do
  @moduledoc """
  Jido-facing Site runtime agent.
  """

  alias Maiden.SiteRuntime.Actions.{
    BeginConstruction,
    CloseSite,
    Commission,
    Decommission,
  ...
  alias Maiden.SiteRuntime.Boundaries
  alias Maiden.SiteRuntime.Directives.{EnqueueTransitionJob, PersistTransition}
  ...
  use Jido.Agent,
    name: "site_runtime_agent",
    description: "Site lifecycle runtime agent with signal-first strategy",
    schema: [
      site_id: [type: :string],
      name: [type: :string],
      status: [type: :string],
      timezone: [type: :string],
      address: [type: :any, default: nil],
      city: [type: :any, default: nil],
      state: [type: :any, default: nil],
      country: [type: :any, default: nil],
      postal_code: [type: :any, default: nil],
    ...
    signal_routes: [
      {"site.transition.begin_construction", BeginConstruction},
      {"site.transition.commission", Commission},
      {"site.transition.seasonal_shutdown", SeasonalShutdown},
      {"site.transition.reopen", Reopen},
      {"site.transition.close", CloseSite},
      {"site.transition.decommission", Decommission},
      {"site.transition.rejected", ObserveRejectedTransition}
    ],
    strategy:
      {SignalFsm,
       initial_state: "idle",
       ...
       bridge_jobs?: true,
       job_queue: :site_runtime_transition}
  ...
  def preflight_agent_state(payload, opts \\ []), do: SiteValidator.agent_state_validate(payload, opts)
  ...
  defp normalize_transition_payload(payload) when is_map(payload) do
    %{}
    |> put_if_present(:site_id, payload)
  ...
  defp normalize_state_key("timezone"), do: :timezone
  defp normalize_state_key("address"), do: :address
  defp normalize_state_key("city"), do: :city
  defp normalize_state_key("state"), do: :state
  defp normalize_state_key("country"), do: :country
  defp normalize_state_key("postal_code"), do: :postal_code
