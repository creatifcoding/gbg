...
function useNavigateNext() {
  const setDir = useAtomSet(navDirectionAtom)
  const setMonth = useAtomSet(viewMonthAtom)
  const setYear = useAtomSet(viewYearAtom)
  return useCallback(() => {
    setDir(1)
    setMonth((m) => {
...
function useSelectDay() {
  const setSelected = useAtomSet(selectedDatesAtom)
...
export function Calendar({
  ...
  const initialValues = React.useMemo((): Iterable<readonly [any, any]> => [
