defmodule Maiden.PlantRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for plant transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"plant.runtime.strategy.tick", {:strategy_tick}, 90},
      {"plant.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           plant_id: fetch(params, :plant_id) || agent.state[:plant_id],
           from: from,
           to: to,
           at: at,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :plant_runtime_transition)]
          }
        ]
    else
      directives
