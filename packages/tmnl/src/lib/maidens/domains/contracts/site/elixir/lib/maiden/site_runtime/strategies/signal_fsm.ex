defmodule Maiden.SiteRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for Site transitions.

  Design:
  - Delegates transition execution semantics to `Jido.Agent.Strategy.FSM`.
  - Emits boundary directives after successful transition execution.
  - Keeps side-effects externalized via directives.
  """
  ...
  def signal_routes(_ctx) do
    [
      {"site.runtime.strategy.tick", {:strategy_tick}, 90},
      {"site.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end

  @spec transition_directives(map(), keyword()) :: list()
  def transition_directives(payload, opts \\ []) when is_map(payload) do
    case Maiden.SiteRuntime.FSM.validate_transition_for_jido(payload) do
      :ok ->
        metadata = %{
          strategy: __MODULE__,
        ...
        []
        |> maybe_add_persist_directive(payload, metadata, opts)
        |> maybe_add_job_directive(payload, opts)
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           site_id: fetch(params, :site_id) || agent.state[:site_id],
           from: from,
           to: to,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :site_runtime_transition)]
          }
        ]
    else
      directives
