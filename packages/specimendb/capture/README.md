...
bun run capture:zip      # writes specimendb-capture.zip
```

Or zip by hand:

```bash
cd packages/specimendb/capture
zip -r ../specimendb-capture.zip .
```
...
Intake still owns filing. Drop this JPEG into specimendb intake. Locality is those GPS tags, or unknown.
