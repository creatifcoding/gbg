defmodule Maiden.SensorAssetRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for sensor-asset transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"sensor_asset.runtime.strategy.tick", {:strategy_tick}, 90},
      {"sensor_asset.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :sensor_asset_runtime_transition)]
          }
        ]
    else
      directives
