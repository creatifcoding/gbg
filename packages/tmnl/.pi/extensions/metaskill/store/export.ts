...
import { RlmStore, type StoredObject } from "./service.ts"
    ...
    return ExportService.of({
      ...
      exportStore: (opts) => Effect.gen(function*() {
        ...
        for (const colName of targetCollectionNames) {
          ...
          for (const item of items) {
            ...
            if (sinceTimestamp && item.updated_at && item.updated_at <= sinceTimestamp) continue
      ...
      importStore: (opts) => Effect.gen(function*() {
        ...
        for (const obj of filteredObjects) {
          try {
            ...
            if (profileName) {
              ...
              data = {
                ...data,
                _meta: { ...existingMeta, profile: profileName },
              }
            }
        ...
        if (profileName && imported > 0) {
          ...
          yield* store.put(PROFILES_NS, profileName, record as unknown as Record<string, unknown>, {
      ...
      removeProfile: (name) => Effect.gen(function*() {
        ...
        for (const addr of data.objects) {
          ...
          const current = yield* store.get(parsed.collection, parsed.key)
          if (current && objectHasProfile(current, name)) {
            yield* store.delete(parsed.collection, parsed.key)
        ...
        // Remove ledger entry
        yield* store.delete(PROFILES_NS, name)
