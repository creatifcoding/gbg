-- Generated from terrarium/BOM.md. Do not invent SKUs, vendors, prices, or lead times.
-- Empty manufacturer_sku / supplier_party / quote / purchase_order is correct.
-- B0371 is a rejected alternate with no PN.

INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B01$bom$, $bom$PETG/ASA corner block$bom$, $bom$8$bom$, $bom$REF$bom$, $bom$$bom$) ON CONFLICT (balloon_id) DO NOTHING;
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B02$bom$, $bom$250 mm edge member$bom$, $bom$16$bom$, $bom$REF$bom$, $bom$$bom$) ON CONFLICT (balloon_id) DO NOTHING;
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B03$bom$, $bom$250-to-500 splice/alignment block$bom$, $bom$4$bom$, $bom$REF$bom$, $bom$$bom$) ON CONFLICT (balloon_id) DO NOTHING;
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B04$bom$, $bom$cassette retainer/gasket carrier$bom$, $bom$set$bom$, $bom$REF$bom$, $bom$$bom$) ON CONFLICT (balloon_id) DO NOTHING;
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B05$bom$, $bom$3 mm cast-acrylic view cassette$bom$, $bom$set$bom$, $bom$REF$bom$, $bom$$bom$) ON CONFLICT (balloon_id) DO NOTHING;
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B06$bom$, $bom$3 mm cast-acrylic front door$bom$, $bom$1$bom$, $bom$REF$bom$, $bom$$bom$) ON CONFLICT (balloon_id) DO NOTHING;
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B07$bom$, $bom$printed door labyrinth surround$bom$, $bom$1$bom$, $bom$REF$bom$, $bom$$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B11$bom$, $bom$<=0.8 mm plastic/fiberglass/polyester screen$bom$, $bom$set$bom$, $bom$LOCK$bom$, $bom$constraint$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B18$bom$, $bom$external 38 mm hybrid rail channel$bom$, $bom$route set$bom$, $bom$REF$bom$, $bom$$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B42$bom$, $bom$Particle Tachyon$bom$, $bom$1$bom$, NULL, $bom$Vendor-documented 85 x 56 x 18.5 mm; exact revision CAD still required$bom$) ON CONFLICT (balloon_id) DO NOTHING;
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B43$bom$, $bom$Particle M1 enclosure$bom$, $bom$0-1$bom$, NULL, $bom$M1ENCLEA; vendor-documented 121 x 220 x 69 mm$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B45$bom$, $bom$Analog Devices MAX96717 camera serializer carrier$bom$, $bom$1$bom$, NULL, $bom$device verified; carrier design UNVERIFIED$bom$) ON CONFLICT (balloon_id) DO NOTHING;
