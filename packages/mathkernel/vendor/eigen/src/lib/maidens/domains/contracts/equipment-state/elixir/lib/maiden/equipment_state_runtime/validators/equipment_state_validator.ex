defmodule Maiden.EquipmentStateRuntime.Validators.EquipmentStateValidator do
  @moduledoc """
  Cross-runtime payload validator for the Equipment State domain.
  """

  @equipment_state_schema_path Path.expand("../../../../../schemas/equipment_state.schema.json", __DIR__)

  @transition_schema_path Path.expand(
                            "../../../../../schemas/equipment_state_transition.schema.json",
                            __DIR__
                          )

  @agent_state_schema_path Path.expand(
                             "../../../../../schemas/equipment_state_agent_state.schema.json",
                             __DIR__
                           )

  @external_resource @equipment_state_schema_path
  @external_resource @transition_schema_path
  @external_resource @agent_state_schema_path

  @equipment_state_schema_runtime @equipment_state_schema_path
                                  |> File.read!()
                                  |> Jason.decode!()
  ...
  @spec equipment_state_validate(map(), keyword()) :: :ok | {:error, term()}
  def equipment_state_validate(payload, opts \\ []) when is_map(payload) do
    validate(payload, :equipment_state, Keyword.get(opts, :engine, :auto))
  end
  ...
  defp validate(payload, :equipment_state, :ex_json_schema),
    do:
      normalize_ex_json_schema(
        ExJsonSchema.Validator.validate(@equipment_state_schema_runtime, payload)
      )
