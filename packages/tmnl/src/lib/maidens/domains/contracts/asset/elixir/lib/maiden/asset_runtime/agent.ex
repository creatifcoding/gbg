defmodule Maiden.AssetRuntime.Agent do
  @moduledoc """
  Jido-facing asset runtime agent.

  ISA-95 alignment:
  - Supports enterprise → site → area → plant → line → workcell → machine semantics
  - Allows sensor/device leaves beneath machine.
  """

  alias Maiden.AssetRuntime.Actions.{ObserveRejectedTransition, SetAssetStatus}
  alias Maiden.AssetRuntime.Boundaries
  alias Maiden.AssetRuntime.Directives.{EnqueueTransitionJob, PersistTransition}
  ...
  use Jido.Agent,
    name: "asset_runtime_agent",
    description: "Asset lifecycle runtime agent with signal-first strategy",
    schema: [
      ...
      properties: [type: :map, default: %{}],
      metadata: [type: :map, default: %{}],
      parent_id: [type: :any, default: nil],
      hierarchy_path: [type: :any, default: nil],
    ...
    signal_routes: [
      {"asset.transition.active", SetAssetStatus},
      {"asset.transition.inactive", SetAssetStatus},
      {"asset.transition.maintenance", SetAssetStatus},
      {"asset.transition.decommissioned", SetAssetStatus},
      {"asset.transition.rejected", ObserveRejectedTransition}
    ],
    strategy:
      {SignalFsm,
       initial_state: "idle",
       ...
       bridge_jobs?: true,
       job_queue: :asset_runtime_transition}

  @transition_actions [SetAssetStatus]
  ...
  def preflight_agent_state(payload, opts \\ []),
    do: AssetValidator.agent_state_validate(payload, opts)
  ...
  defp normalize_transition_payload(payload) when is_map(payload) do
    %{}
    |> put_if_present(:asset_id, payload)
    |> put_if_present(:kind, payload)
  ...
  defp normalize_state_key("kind"), do: :kind
  ...
  defp normalize_state_key("properties"), do: :properties
  defp normalize_state_key("metadata"), do: :metadata
  defp normalize_state_key("parent_id"), do: :parent_id
