defmodule Maiden.MachineAssetRuntime.Sensors.TransitionSensor do
  @moduledoc """
  Sensor ingress adapter for machine-asset transition events.
  ...
  use Jido.Sensor,
    name: "machine_asset_transition_sensor",
    description: "Converts machine-asset transition events into Jido transition signals",
    schema: Zoi.object(
      %{
        source: Zoi.string() |> Zoi.default("/sensor/machine-asset-transition"),
  ...
  @impl true
  def handle_event({:machine_asset_transition, payload}, state) when is_map(payload) do
    ...
    case Agent.preflight_transition(normalized) do
      ...
      {:error, reason} = error ->
        if state.config.emit_rejections do
          rejection_signal =
            Jido.Signal.new!(
              "machine_asset.transition.rejected",
  ...
  def handle_event(payload, state) when is_map(payload) do
    handle_event({:machine_asset_transition, payload}, state)
  end

  def handle_event(_event, state), do: {:ok, state}

  defp signal_type(payload), do: "machine_asset.transition." <> payload["to"]
  ...
  defp attempted_signal_type(payload) do
    case Map.get(payload, "to") do
      to when is_binary(to) and to != "" -> "machine_asset.transition." <> to
      _ -> "machine_asset.transition.unknown"
    end
  end
