...
import {
  ...
  type SessionEntry,
  type ProjectContext,
} from '../../../../.pi/extensions/metaskill/history.ts'
...
describe('HistoryManager', () => {
  ...
  describe('record', () => {
    ...
    it('does not truncate result at exactly RESULT_TRUNCATE_LENGTH', () => {
      ...
      expect(entry.result.length).toBe(RESULT_TRUNCATE_LENGTH)
      expect(entry.result).not.toEndWith('...')
    })
...
describe('Constants', () => {
  it('HISTORY_CUSTOM_TYPE is ms-history', () => {
    expect(HISTORY_CUSTOM_TYPE).toBe('ms-history')
  })
