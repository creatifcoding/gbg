...
// =============================================================================
// TACTICAL UI COMPONENT LIBRARY - Cognitive Electronic Warfare Design System
// =============================================================================
...
const ID = ({ children, className }: { children: ReactNode; className?: string }) => (
  <span className={cn("text-[18px] font-mono font-light tracking-[0.08em] text-white", className)}>{children}</span>
)
...
const GroupHeader = ({
  ...
  return (
    <div
      className={cn("text-[18px] font-mono font-medium tracking-tight text-white flex items-center gap-3", className)}
    >
...
const Button = React.forwardRef<
  ...
  React.ButtonHTMLAttributes<HTMLButtonElement> & {
    variant?: "primary" | "ghost" | "danger" | "outline" | "tactical"
    size?: "xs" | "sm" | "md" | "icon"
  }
>(({ className, variant = "primary", size = "md", children, ...props }, ref) => {
  const variants = {
    ...
    outline: "bg-transparent border-neutral-800 text-neutral-500 hover:border-white hover:text-white",
  ...
  return (
    <button
      ref={ref}
      className={cn(
        "font-mono uppercase tracking-[0.15em] border transition-all relative overflow-hidden active:translate-y-[1px]",
        variants[variant],
        sizes[size],
        className,
      )}
...
const Skeleton = ({ className }: { className?: string }) => (
  <div className={cn("animate-pulse bg-neutral-900 border border-neutral-800", className)} />
)
...
const RadioGroup = ({
  ...
  <div className="flex flex-col gap-2">
    {options.map((opt) => (
      <button key={opt} onClick={() => onChange(opt)} className="flex items-center gap-2 group">
        <div
          className={cn(
...
const DrawerRoot = ({
  ...
  side = "left",
  width = "w-64",
}: { children: ReactNode; open: boolean; onClose: () => void; side?: DrawerSide; width?: string }) => {
          ...
          <>
            ...
            <motion.div
              className={cn(
                ...
                side === "left" ? "left-0 border-r border-neutral-800" : "right-0 border-l border-neutral-800",
                width,
              )}
              initial={slideFrom}
              animate={slideTo}
...
const DrawerContent = ({ children, className }: { children: ReactNode; className?: string }) => (
  <div className={cn("flex-1 overflow-y-auto p-4 no-scrollbar", className)}>{children}</div>
)
...
const DrawerNav = ({
  ...
  activeIndex = 0,
  onSelect,
}: { items: string[]; activeIndex?: number; onSelect?: (i: number) => void }) => (
...
const LeftDrawer = ({
  ...
  onClose,
  width = "w-72",
}: { children: ReactNode; open: boolean; onClose: () => void; width?: string }) => (
...
const CommandBar = ({
  ...
  onClose,
  onExecute,
}: { open: boolean; onClose: () => void; onExecute?: (cmd: string) => void }) => {
...
const Card = ({ children, className, label }: { children: ReactNode; className?: string; label?: string }) => (
...
const StatusFooter = ({
  ...
  value,
  unit,
}: { status?: "connected" | "disconnected" | "warning"; message?: string; value?: number | string; unit?: string }) => {
  const statusColors = { connected: "bg-emerald-500", disconnected: "bg-neutral-600", warning: "bg-amber-500" }
  return (
    <footer className="h-8 border-t border-neutral-800 flex items-center justify-between px-4 bg-black shrink-0">
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1.5">
          <div className={cn("w-1.5 h-1.5 animate-pulse", statusColors[status])} />
      ...
      {value !== undefined && (
        <div className="flex items-baseline gap-1">
          <span className="text-[14px] font-mono text-white font-light">{value}</span>
...
function TacticalKnob({
  ...
  size = 40,
  label,
}: { value: number; min?: number; max?: number; onChange: (v: number) => void; size?: number; label?: string }) {
  ...
  const handleMove = useCallback(
    (clientY: number) => {
      ...
      const newValue = Math.max(min, Math.min(max, startValue.current + (delta / 100) * (max - min)))
      onChange(newValue)
    },
    [min, max, onChange],
  )
    ...
    <div className="flex flex-col items-center gap-1">
      ...
      <div
        ...
        onPointerDown={(e) => {
          ...
          const onMove = (ev: PointerEvent) => handleMove(ev.clientY)
          const onUp = () => {
            handleEnd()
      ...
      >
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-neutral-950 border border-neutral-800"
          style={{ width: size, height: size }}
        >
          <div className="absolute top-1 left-1 right-1 bottom-1 rounded-full bg-black border border-neutral-700">
            <div
              className="absolute top-1 left-1 right-1 bottom-1 rounded-full bg-neutral-900 border border-neutral-600 flex items-center justify-center transition-transform"
              style={{ transform: `rotate(${rotation}deg)` }}
            >
              <div className="absolute top-1.5 w-0.5 h-2 bg-white rounded-full" />
...
function TacticalSlider({
  ...
  const handleInteraction = useCallback(
    (clientX: number) => {
      ...
      onChange(Math.max(min, Math.min(max, Math.round(rawValue / step) * step)))
    },
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      ...
      <div
        className="relative h-6 cursor-pointer touch-none flex items-center"
        onPointerDown={(e) => {
          e.preventDefault()
      ...
      >
        <div ref={trackRef} className="absolute left-0 right-0 h-1 bg-neutral-900 border border-neutral-800">
          <div
            className="absolute top-0 left-0 bottom-0 bg-neutral-600 border-r border-neutral-500"
            style={{ width: `${percent}%` }}
          />
        </div>
        <div
          className="absolute w-4 h-4 -translate-x-1/2 bg-neutral-950 border border-neutral-700"
          style={{ left: `${percent}%` }}
        >
          <div className="absolute inset-0.5 bg-neutral-800 border border-neutral-600">
...
function TacticalFader({
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      >
        <div ref={trackRef} className="absolute top-0 bottom-0 w-1 bg-neutral-900 border border-neutral-800">
          <div
            className="absolute left-0 right-0 bottom-0 bg-neutral-600 border-t border-neutral-500"
            style={{ height: `${percent}%` }}
          />
        </div>
        <div
          className="absolute w-5 h-3 bg-neutral-950 border border-neutral-700"
          style={{ bottom: `${percent}%`, transform: `translateY(50%)` }}
        >
          <div className="absolute inset-0.5 bg-neutral-800 border border-neutral-600 flex items-center justify-center">
