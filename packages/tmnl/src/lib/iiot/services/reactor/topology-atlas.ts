...
const eventOverrides: Record<string, EventCoverageSeed> = {
  ...
  AlarmTriggered: {
    status: 'candidate',
    rationale: 'Guarded opt-in lane: critical/emergency device-subject alarms apply WorkOrder safety holds over targets/requires with SQL safety constraints.',
  ...
  AlarmCleared: {
    status: 'candidate',
    rationale: 'Guarded opt-in lane: alarm clearing retracts only the safety hold addressed by alarmId and lets WorkOrder release logic decide whether resume is legal.',
  ...
  AlarmEscalated: {
    status: 'candidate',
    rationale: 'Guarded opt-in lane: escalation can assert the same alarmId-addressed safety hold for related WorkOrders.',
