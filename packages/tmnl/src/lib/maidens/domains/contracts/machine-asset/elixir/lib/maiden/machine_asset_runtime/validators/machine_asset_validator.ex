defmodule Maiden.MachineAssetRuntime.Validators.MachineAssetValidator do
  @moduledoc """
  Cross-runtime payload validator for the Machine Asset domain.
  """

  @machine_asset_schema_path Path.expand("../../../../../schemas/machine_asset.schema.json", __DIR__)

  @transition_schema_path Path.expand(
                            "../../../../../schemas/machine_asset_transition.schema.json",
                            __DIR__
                          )

  @agent_state_schema_path Path.expand(
                             "../../../../../schemas/machine_asset_agent_state.schema.json",
                             __DIR__
                           )

  @external_resource @machine_asset_schema_path
  @external_resource @transition_schema_path
  @external_resource @agent_state_schema_path

  @machine_asset_schema_runtime @machine_asset_schema_path
                                |> File.read!()
                                |> Jason.decode!()
  ...
  @spec machine_asset_validate(map(), keyword()) :: :ok | {:error, term()}
  def machine_asset_validate(payload, opts \\ []) when is_map(payload) do
    validate(payload, :machine_asset, Keyword.get(opts, :engine, :auto))
  end
  ...
  defp validate(payload, :machine_asset, :ex_json_schema),
    do:
      normalize_ex_json_schema(
        ExJsonSchema.Validator.validate(@machine_asset_schema_runtime, payload)
      )
