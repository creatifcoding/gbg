defmodule Maiden.LineRuntime.Sensors.TransitionSensor do
  @moduledoc """
  Sensor ingress adapter for line transition events.
  ...
  use Jido.Sensor,
    name: "line_transition_sensor",
    description: "Converts line transition events into Jido transition signals",
    schema: Zoi.object(
      %{
        source: Zoi.string() |> Zoi.default("/sensor/line-transition"),
  ...
  @impl true
  def handle_event({:line_transition, payload}, state) when is_map(payload) do
  ...
  def handle_event(payload, state) when is_map(payload) do
    handle_event({:line_transition, payload}, state)
  end

  def handle_event(_event, state), do: {:ok, state}

  defp signal_type(payload), do: "line.transition." <> payload["to"]

  defp normalize_payload(payload) do
    %{}
    |> put_if_present(payload, "line_id", :line_id)
  ...
  defp to_action_params(payload) do
    %{}
    |> put_action_if_present(payload, :line_id)
  ...
  defp rejection_payload(payload, reason, trace_id) do
    %{
      line_id: Map.get(payload, "line_id"),
  ...
  defp attempted_signal_type(payload) do
    case Map.get(payload, "to") do
      to when is_binary(to) and to != "" -> "line.transition." <> to
      _ -> "line.transition.unknown"
    end
  end
