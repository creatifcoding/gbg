defmodule Maiden.DeviceRuntime.Agent do
  @moduledoc """
  Jido-facing device runtime agent.

  ISA-95 alignment:
  - Device is the control-module lane under machine/work-cell hierarchy.
  """

  require Logger

  alias Maiden.DeviceRuntime.Actions.{ObserveRejectedTransition, SetDeviceStatus}
  alias Maiden.DeviceRuntime.Boundaries
  alias Maiden.DeviceRuntime.Directives.{EnqueueTransitionJob, PersistTransition}
  ...
  use Jido.Agent,
    name: "device_runtime_agent",
    description: "Device lifecycle runtime agent with signal-first strategy",
    schema: [
      device_id: [type: :string],
      name: [type: :string],
      status: [type: :string],
      device_type: [type: :string],
      control_mode: [type: :any, default: nil],
      rated_power: [type: :any, default: nil],
      power_unit: [type: :any, default: nil],
      last_command_at: [type: :any, default: nil],
    ...
    signal_routes: [
      {"device.transition.provisioned", SetDeviceStatus},
      {"device.transition.online", SetDeviceStatus},
      {"device.transition.offline", SetDeviceStatus},
      {"device.transition.faulted", SetDeviceStatus},
      {"device.transition.firmware_update", SetDeviceStatus},
      {"device.transition.decommissioned", SetDeviceStatus},
      {"device.transition.rejected", ObserveRejectedTransition}
    ],
    strategy:
      {SignalFsm,
       # Strategy FSM models instruction execution flow; canonical domain
       # transitions are enforced by Maiden.DeviceRuntime.FSM.
       initial_state: "idle",
       transitions: %{
         "idle" => ["processing"],
       ...
       bridge_jobs?: true,
       job_queue: :device_runtime_transition}

  @transition_actions [SetDeviceStatus]
  ...
  def preflight_agent_state(payload, opts \\ []),
    do: DeviceValidator.agent_state_validate(payload, opts)
  ...
  defp execute_runtime_directive(agent, %PersistTransition{event: event, metadata: metadata}) do
    case Boundaries.persist_transition(event, metadata, agent_id: agent.id) do
      :ok ->
        {:ok, agent, []}

      {:error, reason} ->
        Logger.warning("device boundary persist_transition failed during sync directive resolution: #{inspect(reason)}")
        {:ok, agent, []}
    end
  end

  defp execute_runtime_directive(agent, %EnqueueTransitionJob{event: event, opts: opts}) do
    merged_opts = Keyword.put_new(opts || [], :agent_id, agent.id)
    ...
    case Boundaries.enqueue_transition(event, merged_opts) do
      ...
      {:error, reason} ->
        Logger.warning("device boundary enqueue_transition failed during sync directive resolution: #{inspect(reason)}")
        {:ok, agent, []}
    end
  end
  ...
  defp normalize_transition_payload(payload) when is_map(payload) do
    %{}
    |> put_if_present(:device_id, payload)
  ...
  defp normalize_state_key("device_type"), do: :device_type
  defp normalize_state_key("control_mode"), do: :control_mode
  defp normalize_state_key("rated_power"), do: :rated_power
  defp normalize_state_key("power_unit"), do: :power_unit
  defp normalize_state_key("last_command_at"), do: :last_command_at
