defmodule Maiden.PlantRuntime.Validators.PlantValidator do
  @moduledoc """
  Cross-runtime payload validator for the Plant domain.

  Mirrors TypeScript Effect Schema artifacts generated under `../schemas`.
  """

  @plant_schema_path Path.expand("../../../../../schemas/plant.schema.json", __DIR__)
  @transition_schema_path Path.expand("../../../../../schemas/plant_transition.schema.json", __DIR__)
  @agent_state_schema_path Path.expand("../../../../../schemas/plant_agent_state.schema.json", __DIR__)

  @external_resource @plant_schema_path
  @external_resource @transition_schema_path
  @external_resource @agent_state_schema_path

  @plant_schema_runtime (
    @plant_schema_path
    |> File.read!()
    |> Jason.decode!()
  ...
  @spec plant_validate(map(), keyword()) :: :ok | {:error, term()}
  def plant_validate(payload, opts \\ []) when is_map(payload) do
    validate(payload, :plant, Keyword.get(opts, :engine, :auto))
  end
  ...
  defp validate(payload, :plant, :ex_json_schema),
    do: normalize_ex_json_schema(ExJsonSchema.Validator.validate(@plant_schema_runtime, payload))
