defmodule Maiden.WorkcellRuntime.Validators.WorkcellValidator do
  @moduledoc """
  Cross-runtime payload validator for the WorkCell domain.
  """

  @workcell_schema_path Path.expand("../../../../../schemas/workcell.schema.json", __DIR__)
  @transition_schema_path Path.expand("../../../../../schemas/workcell_transition.schema.json", __DIR__)
  @agent_state_schema_path Path.expand("../../../../../schemas/workcell_agent_state.schema.json", __DIR__)

  @external_resource @workcell_schema_path
  @external_resource @transition_schema_path
  @external_resource @agent_state_schema_path

  @workcell_schema_runtime (
                             @workcell_schema_path
                             |> File.read!()
                             |> Jason.decode!()
  ...
  @spec workcell_validate(map(), keyword()) :: :ok | {:error, term()}
  def workcell_validate(payload, opts \\ []) when is_map(payload) do
    validate(payload, :workcell, Keyword.get(opts, :engine, :auto))
  end
  ...
  defp validate(payload, :workcell, :ex_json_schema),
    do: normalize_ex_json_schema(ExJsonSchema.Validator.validate(@workcell_schema_runtime, payload))
