defmodule Maiden.DeviceRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for device transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"device.runtime.strategy.tick", {:strategy_tick}, 90},
      {"device.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           device_id: fetch(params, :device_id) || agent.state[:device_id],
           from: from,
           to: to,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :device_runtime_transition)]
          }
        ]
    else
      directives
