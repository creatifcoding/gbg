defmodule Maiden.PlantRuntime.Agent do
  @moduledoc """
  Jido-facing plant runtime agent.

  ISA-95 alignment:
  - Plant is the Site/Area scoped parent of production lines.
  """

  alias Maiden.PlantRuntime.Actions.{ObserveRejectedTransition, SetPlantStatus}
  alias Maiden.PlantRuntime.Boundaries
  alias Maiden.PlantRuntime.Directives.{EnqueueTransitionJob, PersistTransition}
  ...
  use Jido.Agent,
    name: "plant_runtime_agent",
    description: "Plant lifecycle runtime agent with signal-first strategy",
    schema: [
      plant_id: [type: :string],
      name: [type: :string],
      status: [type: :string],
      timezone: [type: :string],
      site_code: [type: :any, default: nil],
    ...
    signal_routes: [
      {"plant.transition.commissioning", SetPlantStatus},
      {"plant.transition.operational", SetPlantStatus},
      {"plant.transition.scheduled_shutdown", SetPlantStatus},
      {"plant.transition.emergency_shutdown", SetPlantStatus},
      {"plant.transition.maintenance_shutdown", SetPlantStatus},
      {"plant.transition.decommissioned", SetPlantStatus},
      {"plant.transition.rejected", ObserveRejectedTransition}
    ],
    strategy:
      {SignalFsm,
       initial_state: "idle",
       ...
       bridge_jobs?: true,
       job_queue: :plant_runtime_transition}

  @transition_actions [SetPlantStatus]
  ...
  def preflight_agent_state(payload, opts \\ []),
    do: PlantValidator.agent_state_validate(payload, opts)
  ...
  defp normalize_transition_payload(payload) when is_map(payload) do
    %{}
    |> put_if_present(:plant_id, payload)
  ...
  defp normalize_state_key("site_code"), do: :site_code
