defmodule Maiden.MachineAssetRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for machine-asset transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"machine_asset.runtime.strategy.tick", {:strategy_tick}, 90},
      {"machine_asset.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :machine_asset_runtime_transition)]
          }
        ]
    else
      directives
