defmodule Maiden.EnterpriseRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for Enterprise transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"enterprise.runtime.strategy.tick", {:strategy_tick}, 90},
      {"enterprise.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           enterprise_id: fetch(params, :enterprise_id) || agent.state[:enterprise_id],
           from: from,
           to: to,
           at: at,
  ...
  defp maybe_add_job_directive(directives, event, opts) do
    if Keyword.get(opts, :bridge_jobs?, true) do
      directives ++
        [%EnqueueTransitionJob{event: event, opts: [queue: Keyword.get(opts, :job_queue, :enterprise_runtime_transition)]}]
    else
      directives
    end
  end
