defmodule Maiden.AreaRuntime.Sensors.TransitionSensor do
  @moduledoc """
  Sensor ingress adapter for area transition events.
  ...
  use Jido.Sensor,
    name: "area_transition_sensor",
    description: "Converts area transition events into Jido transition signals",
    schema: Zoi.object(
      %{
        source: Zoi.string() |> Zoi.default("/sensor/area-transition"),
  ...
  @impl true
  def handle_event({:area_transition, payload}, state) when is_map(payload) do
  ...
  def handle_event(payload, state) when is_map(payload) do
    handle_event({:area_transition, payload}, state)
  end

  def handle_event(_event, state), do: {:ok, state}

  defp signal_type(payload), do: "area.transition." <> payload["to"]

  defp normalize_payload(payload) do
    %{}
    |> put_if_present(payload, "area_id", :area_id)
  ...
  defp to_action_params(payload) do
    %{}
    |> put_action_if_present(payload, :area_id)
  ...
  defp attempted_signal_type(payload) do
    case Map.get(payload, "to") do
      to when is_binary(to) and to != "" -> "area.transition." <> to
      _ -> "area.transition.unknown"
    end
  end
