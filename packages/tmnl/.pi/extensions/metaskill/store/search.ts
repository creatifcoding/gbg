...
import { namespaceMatchesGlob } from "./schemas.ts"
import { RlmStore, type CatalogEntry } from "./service.ts"
...
export interface SearchIndexShape {
   ...
   */
  readonly search: (text: string, nsGlob?: string) => Effect.Effect<readonly SearchHit[]>

  /**
   * Notify the index that an object was added/updated.
   * Called by the store on put operations.
   */
  readonly notify: (ns: string, key: string, data: Record<string, unknown>, tags: readonly string[]) => Effect.Effect<void>

  /**
   * Notify the index that an object was removed.
   */
  readonly notifyRemove: (ns: string, key: string) => Effect.Effect<void>

  /**
   * Rebuild the entire FlexSearch index from SQLite.
   */
  readonly rebuild: () => Effect.Effect<{ indexed: number }>

  /**
   * Get index stats.
   */
  readonly stats: () => Effect.Effect<{ flexCount: number; ready: boolean }>
}
        ...
        Effect.gen(function*() {
          ...
          if (isReady) {
            ...
            return yield* searchFts5(text, nsGlob).pipe(
              Effect.catchAll(() => Effect.succeed([] as readonly SearchHit[])),
            )
          }
