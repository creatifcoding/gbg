defmodule Maiden.EnterpriseRuntime.Validators.EnterpriseValidator do
  @moduledoc """
  Cross-runtime payload validator for the Enterprise domain.
  """

  @enterprise_schema_path Path.expand("../../../../../schemas/enterprise.schema.json", __DIR__)

  @transition_schema_path Path.expand(
                            "../../../../../schemas/enterprise_transition.schema.json",
                            __DIR__
                          )

  @agent_state_schema_path Path.expand(
                             "../../../../../schemas/enterprise_agent_state.schema.json",
                             __DIR__
                           )

  @external_resource @enterprise_schema_path
  @external_resource @transition_schema_path
  @external_resource @agent_state_schema_path

  @enterprise_schema_runtime @enterprise_schema_path
                             |> File.read!()
                             |> Jason.decode!()
  ...
  @spec enterprise_validate(map(), keyword()) :: :ok | {:error, term()}
  def enterprise_validate(payload, opts \\ []) when is_map(payload) do
    validate(payload, :enterprise, Keyword.get(opts, :engine, :auto))
  end
  ...
  defp validate(payload, :enterprise, :ex_json_schema),
    do: normalize_ex_json_schema(ExJsonSchema.Validator.validate(@enterprise_schema_runtime, payload))
