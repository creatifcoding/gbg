...
Reads terrarium/ee/kicad/libs and contracts (does not rewrite them). Coupon net
names are frozen from closed PR 40 @ a9918b32 — #24/#25 boards are absent on
feat/mantis-biomemetics-lab and are not recreated. Does not invent MPNs. MIPI
stays local to courtyard-only SerDes envelopes. Complete coupon: ser launch,
B50, carriage, B27/B19, receiver, adjacent power, S1/S2/Q1, cal/OSL, safe fault
insertion. PROTO-FAB DRAFT / UNQUALIFIED.
"""
...
KICAD = HERE.parent
TERR = HERE.parents[2]
CONTRACT = KICAD / "contracts" / "ee-interface-v1.json"
SOURCE_COUPON_REF = "closed PR 40 / cursor/mantis-ee-26-proto-fab-376a @ a9918b32"
...
# Frozen #25 B27/B50 names from PR40 coupon facts. Coupon nets are prefixed so
# structures do not load each other. Do not invent alternate pinouts.
B27_ROLES = {
    "P01": "VIN_A",
    "P02": "VIN_B",
    "P03": "GND_A",
...
# #24 power net names frozen from PR40 (boards not on this base).
POWER_NETS_FROM_24 = [
    "VIN_RAIL",
    "VIN_FUSED",
    "VIN_SHARED",
...
# Parent board net-maps are absent on feat/mantis-biomemetics-lab. Facts only.
PARENT_FACTS = {
    "power-control": {
        "status": "UNVERIFIED",
        "issue": 24,
        "presence": "absent-on-base",
        "source": SOURCE_COUPON_REF,
        "note": "net names frozen; board not recreated",
    },
    "camera-tx": {
        "status": "UNVERIFIED",
        "issue": 25,
...
def _norm_net(name: str) -> str:
    """Map contract display nets to coupon underscore roles without inventing pins."""
    n = name.strip().upper()
    n = (
        n.replace("GMSL+", "GMSL_P")
        .replace("GMSL−", "GMSL_N")  # unicode minus
        .replace("GMSL-", "GMSL_N")
    )
    n = n.replace("-", "_")
    n = re.sub(r"/.*$", "", n)  # FAULT_N/IRQ -> FAULT_N
    return n


def consume_parents() -> dict:
    """Validate #23 interface contract; freeze #24/#25 board facts from PR40.

    Does not read or recreate camera-tx / carriage / rail-rx / power-control.
    """
    if not CONTRACT.is_file():
        raise SystemExit(f"EE-01 interface contract missing (read-only dependency): {CONTRACT}")
    contract = json.loads(CONTRACT.read_text())
    for pin, role in B27_ROLES.items():
        entry = next((p for p in contract.get("b27", []) if p.get("pin") == pin), None)
        if entry is None:
            raise SystemExit(f"contract missing B27 {pin}")
        if _norm_net(entry.get("net", "")) != role:
            raise SystemExit(
                f"B27 {pin} role mismatch: coupon {role} vs contract {entry.get('net')}"
            )
    for pin, role in B50_ROLES.items():
        entry = next((p for p in contract.get("b50", []) if p.get("pin") == pin), None)
        if entry is None:
            raise SystemExit(f"contract missing B50 {pin}")
        if _norm_net(entry.get("net", "")) != role:
            raise SystemExit(
                f"B50 {pin} role mismatch: coupon {role} vs contract {entry.get('net')}"
            )
    # Refuse to silently adopt on-disk #24/#25 boards if they appear later.
    for leaf in ("power-control", "camera-tx", "carriage", "rail-rx"):
        if (KICAD / leaf / "net-map.json").is_file():
            raise SystemExit(
                f"{leaf}/net-map.json present on base; this reconstituter freezes PR40 "
                "facts and must not recreate or silently rebind those boards"
            )
    return dict(PARENT_FACTS)
...
def main() -> None:
    ...
    note = (
        ...
        "Reconstituted on feat/mantis-biomemetics-lab from PR40 @ a9918b32. #23 libs+contracts on base.\\n"
        "#24/#25 boards absent — net names frozen, not recreated. No invented MPN. No measured SI/PI.\\n"
        "Does not implement #18. Does not touch PR 12 / SpecimenDB. Does not rewrite #23 libs."
    )
    sch = root_sheet(
    ...
    netmap = {
        ...
        "sourceCouponFacts": SOURCE_COUPON_REF,
        "consumes": {
            "library": "terrarium/ee/kicad/libs (EE-01 / #23, envelopes on lab base)",
            "eeInterface": "terrarium/ee/kicad/contracts/ee-interface-v1.json",
            "powerControl": "absent-on-base; #24 net names frozen from PR40 coupon facts",
            "cameraTx": "absent-on-base; #25 net names frozen from PR40 coupon facts",
            "carriage": "absent-on-base; #25 net names frozen from PR40 coupon facts",
            "railRx": "absent-on-base; #25 net names frozen from PR40 coupon facts",
        },
        "parentStatus": {
            k: {"status": v.get("status"), "presence": v.get("presence")}
        ...
        "nodes": {
            ...
            "V_BRANCH": "Q1B output to B50 C01/C02; enable is #24 Q1_EN (name frozen)",
        ...
        "powerNetsFrom24": POWER_NETS_FROM_24,
        "b27": B27_ROLES,
        "b50": B50_ROLES,
