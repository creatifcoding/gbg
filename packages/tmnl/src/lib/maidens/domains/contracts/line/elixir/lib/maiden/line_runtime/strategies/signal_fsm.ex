defmodule Maiden.LineRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for line transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"line.runtime.strategy.tick", {:strategy_tick}, 90},
      {"line.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           line_id: fetch(params, :line_id) || agent.state[:line_id],
           from: from,
           to: to,
           at: at,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :line_runtime_transition)]
          }
        ]
    else
      directives
