...
export function buildInjectedPrompt(
  ...
  const blocks = inject
    .map(ref => {
      ...
      if (!parsed) return ''
      const obj = store.get(parsed.col, parsed.key)
      if (obj == null) return ''
...
export function createLlmBridge(store: StoreAPI, config?: LlmBridgeConfig): LlmBridge {
  ...
  async function llm(prompt: string, opts?: LlmOpts): Promise<string> {
    guardCalls(1)
    const fullPrompt = buildInjectedPrompt(prompt, opts?.inject, store)
  ...
  async function llm_batch(
    ...
    for (let i = 0; i < items.length; i += concurrency) {
      ...
      const settled = await Promise.allSettled(
        batch.map(item => {
          const fullPrompt = buildInjectedPrompt(item.prompt, item.inject, store)
