 ...
 *   bun test src/lib/metaskill/__tests__/store-v2.test.ts
 *   node --experimental-strip-types --test src/lib/metaskill/__tests__/store-v2.test.ts
 *
 * Uses :memory: SQLite for isolation. No filesystem side effects.
 */

import { describe, it, expect, beforeAll, afterAll } from 'bun:test'
import { createStoreApi } from '../../../../.pi/extensions/metaskill/store/api.ts'
import { createFactoryApi } from '../../../../.pi/extensions/metaskill/store/factories.ts'
import type { StoreApi } from '../../../../.pi/extensions/metaskill/store/api.ts'
import type { FactoryApi } from '../../../../.pi/extensions/metaskill/store/factories.ts'
...
async function makeLayer() {
  if (isBun) {
    const { layer } = await import('../../../../.pi/extensions/metaskill/store/sqlite-bun.ts')
    return layer({ filename: ':memory:' })
  } else {
    const { layer } = await import('../../../../.pi/extensions/metaskill/store/sqlite-node.ts')
    return layer({ filename: ':memory:' })
  }
}
