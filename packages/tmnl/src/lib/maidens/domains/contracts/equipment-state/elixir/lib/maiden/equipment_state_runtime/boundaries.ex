defmodule Maiden.EquipmentStateRuntime.Boundaries do
  @moduledoc """
  ...
  alias Maiden.EquipmentStateRuntime.Boundaries.{
    NoopEquipmentStateStore,
    NoopJobQueue
  }

  @app :maiden_equipment_state_runtime
  ...
  def persist_transition(event, metadata, opts \\ []) when is_map(event) and is_map(metadata) do
    adapter = equipment_state_store_adapter()
  ...
  @spec equipment_state_store_adapter() :: module()
  def equipment_state_store_adapter do
    Application.get_env(@app, :equipment_state_store_adapter, NoopEquipmentStateStore)
  end
