defmodule Maiden.DeviceRuntime.Sensors.TransitionSensor do
  @moduledoc """
  Sensor ingress adapter for device transition events.
  ...
  use Jido.Sensor,
    name: "device_transition_sensor",
    description: "Converts device transition events into Jido transition signals",
    schema: Zoi.object(
      %{
        source: Zoi.string() |> Zoi.default("/sensor/device-transition"),
  ...
  @impl true
  def handle_event({:device_transition, payload}, state) when is_map(payload) do
  ...
  def handle_event(payload, state) when is_map(payload) do
    handle_event({:device_transition, payload}, state)
  end

  def handle_event(_event, state), do: {:ok, state}

  defp signal_type(payload), do: "device.transition." <> payload["to"]

  defp normalize_payload(payload) do
    %{}
    |> put_if_present(payload, "device_id", :device_id)
  ...
  defp to_action_params(payload) do
    %{}
    |> put_action_if_present(payload, :device_id)
  ...
  defp rejection_payload(payload, reason, trace_id) do
    %{
      device_id: Map.get(payload, "device_id"),
  ...
  defp attempted_signal_type(payload) do
    case Map.get(payload, "to") do
      to when is_binary(to) and to != "" -> "device.transition." <> to
      _ -> "device.transition.unknown"
    end
  end
