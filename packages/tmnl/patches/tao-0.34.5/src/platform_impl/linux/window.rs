...
impl Window {
  pub(crate) fn new<T>(
    ...
    let default_vbox = if pl_attribs.default_vbox {
      ...
      // Tao/GTK+Wayland sizing is sensitive to children with unset size
      // requests. Upstream Tao PR #984 fixes wrong initial inner_size by
      // explicitly sizing the internal window box. Keep this gated for the
      // TMNL layer-shell path so normal Tao users retain stock behavior.
      if std::env::var_os("TMNL_TAO_SET_VBOX_SIZE_REQUEST").is_some() {
    ...
    if std::env::var_os("TMNL_TAO_SKIP_INITIAL_SET_VISIBLE").is_none() {
