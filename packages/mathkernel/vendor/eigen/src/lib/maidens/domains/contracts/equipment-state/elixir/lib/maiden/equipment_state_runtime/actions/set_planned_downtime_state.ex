defmodule Maiden.EquipmentStateRuntime.Actions.SetPlannedDowntimeState do
  @moduledoc """
  Explicit transition action: <supported> -> planned_downtime.
  """

  use Jido.Action,
    name: "set_planned_downtime_state",
    description: "Set equipment state to planned_downtime",
    schema: [
  ...
  @allowed_from ["running", "idle", "unplanned_downtime", "setup"]
  ...
  def run(params, context) when is_map(params) do
    ...
    cond do
      from not in @allowed_from or to != "planned_downtime" ->
        {:error, "SetPlannedDowntimeState requires to=planned_downtime and valid source state"}
  ...
  def run(_params, _context),
    do:
      {:error,
       "SetPlannedDowntimeState requires to=planned_downtime and valid source state"}
