defmodule Maiden.DeviceRuntime.Validators.DeviceValidator do
  @moduledoc """
  Cross-runtime payload validator for the Device domain.

  Mirrors TypeScript Effect Schema artifacts generated under `../schemas`.
  """

  @device_schema_path Path.expand("../../../../../schemas/device.schema.json", __DIR__)
  @transition_schema_path Path.expand("../../../../../schemas/device_transition.schema.json", __DIR__)
  @agent_state_schema_path Path.expand("../../../../../schemas/device_agent_state.schema.json", __DIR__)

  @external_resource @device_schema_path
  @external_resource @transition_schema_path
  @external_resource @agent_state_schema_path

  @device_schema_runtime (
    @device_schema_path
    |> File.read!()
    |> Jason.decode!()
  ...
  @spec device_validate(map(), keyword()) :: :ok | {:error, term()}
  def device_validate(payload, opts \\ []) when is_map(payload) do
    validate(payload, :device, Keyword.get(opts, :engine, :auto))
  end
  ...
  defp validate(payload, :device, :ex_json_schema),
    do: normalize_ex_json_schema(ExJsonSchema.Validator.validate(@device_schema_runtime, payload))
