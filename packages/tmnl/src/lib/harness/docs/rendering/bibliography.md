...
### [B25] `src/lib/harness/docs/specs/harness-provider-markers-spec.md`
...
## H) External architecture analogs (TipTap + Streamdown)

### [B27] TipTap `ExtensionManager` plugin assembly and priority handling
- URL: https://github.com/ueberdosis/tiptap/blob/main/packages/core/src/ExtensionManager.ts
- Why it matters: explicit ordered plugin composition, extension context binding, and middleware-style dispatch chaining.

### [B28] TipTap extension priority sorter
- URL: https://github.com/ueberdosis/tiptap/blob/main/packages/core/src/helpers/sortExtensions.ts
- Why it matters: deterministic priority policy for extension execution order.

### [B29] TipTap dispatch transaction contract
- URL: https://github.com/ueberdosis/tiptap/blob/main/packages/core/src/types.ts
- Why it matters: formal middleware signature (`transaction`, `next`) for composable interception.

### [B30] TipTap plugin order conformance test
- URL: https://github.com/ueberdosis/tiptap/blob/main/tests/cypress/integration/core/pluginOrder.spec.ts
- Why it matters: verifies actual execution order semantics under mixed priorities.

### [B31] Streamdown streaming/static mode architecture
- URL: https://github.com/vercel/streamdown/blob/main/packages/streamdown/index.tsx
- Why it matters: clean separation of streaming-mode behavior from static rendering path.

### [B32] Streamdown plugin contracts
- URL: https://github.com/vercel/streamdown/blob/main/packages/streamdown/lib/plugin-types.ts
- Why it matters: typed plugin interfaces and phase/order constraints.

### [B33] Streamdown block parser for streaming markdown
- URL: https://github.com/vercel/streamdown/blob/main/packages/streamdown/lib/parse-blocks.tsx
- Why it matters: segmentation strategy for incremental rendering and malformed/incomplete stream handling.

### [B34] Streamdown plugin documentation (ordering + customization)
- URL: https://github.com/vercel/streamdown/blob/main/streamdown/references/plugins.md
- Why it matters: operational guidance for plugin order and composition extension points.

---

## I) New TMNL rendering reducer scaffold

### [B35] `src/lib/harness/rendering/schemas.ts`
- Why it matters: schema-backed data contracts for reducer inputs, patches, render nodes, and emissions.

### [B36] `src/lib/harness/rendering/OverlayReducerPipeline.ts`
- Why it matters: initial fork/join overlay execution + deterministic collector service.

### [B37] `src/lib/harness/docs/rendering/overlay-reducer-pipeline-architecture.md`
- Why it matters: architecture synthesis and rollout plan bridging literature to implementation.
...
5. Use [B27]-[B34] for cross-repo architecture analogs (priority pipelines, middleware chaining, block partitioning).
6. Use [B35]-[B37] as the canonical TMNL overlay-reducer baseline for follow-on implementation slices.
