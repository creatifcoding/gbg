defmodule Maiden.AreaRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for area transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"area.runtime.strategy.tick", {:strategy_tick}, 90},
      {"area.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           area_id: fetch(params, :area_id) || agent.state[:area_id],
           from: from,
           to: to,
           at: at,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :area_runtime_transition)]
          }
        ]
    else
      directives
