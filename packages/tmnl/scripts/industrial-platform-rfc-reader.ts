#!/usr/bin/env bun
/**
 * Generate the Plant Ops Platform RFC guided reader.
 *
...
const outPath = join(homedir(), '.agent/diagrams/plant-ops-platform-rfc-reader.html')

const docs = [
  ['index', 'RFC Pack Index', 'README.md', 'Start here: thesis, doc map, non-negotiables, implementation order.', 'cyan'],
  ['charter', 'RFC-0000 Charter', 'RFC-0000-CHARTER.md', 'Product and architecture charter for the agentic plant-ops digital twin/control plane.', 'amber'],
  ...
  ['decomposition', 'RFC-0011 Plant Ops Decomposition', 'RFC-0011-PLANT-OPS-DECOMPOSITION.md', 'Bounded contexts, dependency DAG, feature-plan map, and implementation train for the Plant Ops Platform.', 'cyan'],
  ['effect-grounding', 'Effect v3 Grounding Ledger', 'EFFECT-V3-GROUNDING-LEDGER.md', 'Effect v3 source-grounded architecture rules for Schema, services, Layers, streams, SQL authority, and ManagedRuntime edges.', 'green'],
  ['namespace-migration', 'Namespace Migration', 'NAMESPACE-MIGRATION.md', 'Migration plan from broad industrial naming to plant-ops paths and terminology.', 'amber'],
  ['boundary-rules', 'Boundary Rules', 'BOUNDARY-RULES.md', 'Import, runtime, adapter, agent, SQL authority, projection, and Reactor guardrails for implementation slices.', 'red'],
...
const render = (items: ReadonlyArray<Doc>): string => {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>Plant Ops Platform RFC Guided Reader · TMNL</title><style>${css}</style></head><body><div class="shell"><nav class="toc"><div class="brand"><div class="mark">λ</div><div><strong>TMNL Reader</strong><small>Plant Ops RFC Pack</small></div></div>${nav}</nav><main><section class="hero"><p class="eyebrow">Guided RFC dossier</p><h1>Plant Ops Platform</h1><p>A reader for the post-Reactor IIoT platform: digital twin, DMN, dependency-injected plant operations integrations, command governance, virtual plant CI, deployment policy, and market wedge strategy.</p><div class="metrics"><div class="metric"><b>${items.length}</b><span>documents</span></div><div class="metric"><b>L3+</b><span>ISA-95 center</span></div><div class="metric"><b>0</b><span>unguarded OT writes</span></div><div class="metric"><b>DI</b><span>all integrations</span></div></div></section><section class="route"><div class="panel"><h2>Recommended reading route</h2><div class="sequence"><div class="step"><b>1</b><div><strong>Approve the thesis.</strong><br/>Index → Charter → Source Ledger. This nails what we are building and what we refuse to become.</div></div><div class="step"><b>2</b><div><strong>Approve the substrate.</strong><br/>Integration Ports → DMN → Effect Grounding → Decomposition. This defines OPC UA, Sparkplug, historian, CMMS/MES/ERP, and PCT/LNK/MSH boundaries.</div></div><div class="step"><b>3</b><div><strong>Approve safe action.</strong><br/>Command Governance → Virtual Plant/Deployment → Boundary Rules → Market Wedges. This turns agent ambition into governed, testable product slices.</div></div></div></div><div class="panel"><h2>Review gate</h2><p><strong>Decision needed:</strong> Is this the right RFC spine for the Plant Ops Platform foundation?</p><p><strong>Watch for:</strong> vendor lock-in, unsafe command ambiguity, fake-vs-real adapter drift, broad namespace sprawl, and anything that tries to sneak around Reactor's source-of-truth discipline.</p></div></section><section class="cards">${cards}</section>${sections}<div class="footer">Generated from <code>${docsRoot}</code>. Reader artifact lives at <code>${outPath}</code>.</div></main></div><script>const links=[...document.querySelectorAll('.toc a')];const sections=[...document.querySelectorAll('[data-doc-section]')];const obs=new IntersectionObserver(entries=>{for(const e of entries){if(e.isIntersecting){links.forEach(l=>l.classList.toggle('active',l.dataset.doc===e.target.dataset.docSection));}}},{rootMargin:'-25% 0px -65% 0px'});sections.forEach(s=>obs.observe(s));</script></body></html>`
}
