defmodule Maiden.AreaRuntime.Agent do
  @moduledoc """
  Jido-facing area runtime agent.

  ISA-95 alignment:
  - Area belongs under Site directly, or under Plant when lane topology requires it.
  """

  alias Maiden.AreaRuntime.Actions.{ObserveRejectedTransition, SetAreaStatus}
  alias Maiden.AreaRuntime.Boundaries
  alias Maiden.AreaRuntime.Directives.{EnqueueTransitionJob, PersistTransition}
  ...
  use Jido.Agent,
    name: "area_runtime_agent",
    description: "Area lifecycle runtime agent with signal-first strategy",
    schema: [
      area_id: [type: :string],
      name: [type: :string],
      status: [type: :string],
      ...
      site_id: [type: :string],
      area_type: [type: :any, default: nil],
      building: [type: :any, default: nil],
      floor: [type: :any, default: nil],
      zone: [type: :any, default: nil],
    ...
    signal_routes: [
      {"area.transition.active", SetAreaStatus},
      {"area.transition.restricted", SetAreaStatus},
      {"area.transition.maintenance", SetAreaStatus},
      {"area.transition.inactive", SetAreaStatus},
      {"area.transition.decommissioned", SetAreaStatus},
      {"area.transition.rejected", ObserveRejectedTransition}
    ],
    strategy:
      {SignalFsm,
       initial_state: "idle",
       ...
       bridge_jobs?: true,
       job_queue: :area_runtime_transition}

  @transition_actions [SetAreaStatus]
  ...
  def preflight_agent_state(payload, opts \\ []),
    do: AreaValidator.agent_state_validate(payload, opts)
  ...
  defp normalize_transition_payload(payload) when is_map(payload) do
    %{}
    |> put_if_present(:area_id, payload)
  ...
  defp normalize_state_key("area_type"), do: :area_type
  defp normalize_state_key("building"), do: :building
  defp normalize_state_key("floor"), do: :floor
  defp normalize_state_key("zone"), do: :zone
