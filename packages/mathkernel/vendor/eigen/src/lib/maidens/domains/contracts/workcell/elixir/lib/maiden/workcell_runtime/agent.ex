defmodule Maiden.WorkcellRuntime.Agent do
  @moduledoc """
  Jido-facing WorkCell runtime agent.
  """

  alias Maiden.WorkcellRuntime.Actions.{ObserveRejectedTransition, SetWorkcellStatus}
  ...
  use Jido.Agent,
    name: "workcell_runtime_agent",
    description: "Workcell lifecycle runtime agent with signal-first strategy",
    schema: [
      workcell_id: [type: :string],
      line_id: [type: :string],
      name: [type: :string],
      status: [type: :string],
      cell_type: [type: :any, default: nil],
      cycle_time_seconds: [type: :any, default: nil],
      position: [type: :any, default: nil],
    ...
    signal_routes: [
      {"workcell.transition.idle", SetWorkcellStatus},
      {"workcell.transition.setup", SetWorkcellStatus},
      {"workcell.transition.running", SetWorkcellStatus},
      {"workcell.transition.blocked", SetWorkcellStatus},
      {"workcell.transition.faulted", SetWorkcellStatus},
      {"workcell.transition.maintenance", SetWorkcellStatus},
      {"workcell.transition.decommissioned", SetWorkcellStatus},
      {"workcell.transition.rejected", ObserveRejectedTransition}
    ],
    strategy:
      {SignalFsm,
       # Strategy FSM models instruction execution flow; canonical domain
       # transitions are enforced by Maiden.WorkcellRuntime.FSM.
       initial_state: "idle",
       transitions: %{
         "idle" => ["processing"],
       ...
       bridge_jobs?: true,
       job_queue: :workcell_runtime_transition}

  @transition_actions [SetWorkcellStatus]
  ...
  def preflight_transition(payload, opts \\ []), do: FSM.validate_transition_for_runtime(payload, opts)

  @spec preflight_agent_state(map(), keyword()) :: :ok | {:error, term()}
  def preflight_agent_state(payload, opts \\ []),
    do: WorkcellValidator.agent_state_validate(payload, opts)
  ...
  defp normalize_transition_payload(payload) when is_map(payload) do
    %{}
    |> put_if_present(:workcell_id, payload)
  ...
  defp normalize_state_key("workcell_id"), do: :workcell_id
  ...
  defp normalize_state_key("cell_type"), do: :cell_type
  defp normalize_state_key("cycle_time_seconds"), do: :cycle_time_seconds
  defp normalize_state_key("position"), do: :position
