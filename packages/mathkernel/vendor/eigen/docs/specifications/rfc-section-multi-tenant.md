...
#### Y.3.3 CAP/PACELC Position per Data Path

The multi-tenant architecture adopts different consistency-availability tradeoffs
depending on the data path. Following the PACELC framework [PACELC], each path
declares its position during both partition (P) and normal operation (E):

| Data Path | ISA-95 Level | If Partition | Else (Normal) | Position | Rationale |
|-----------|-------------|-------------|---------------|----------|-----------|
| Sensor readings | L0 | Consistency (edge continues locally) | Consistency (ordered delivery) | PC/EC | Safety-critical; local JetStream preserves G-1 |
| Alarm detection & notification | L1-L2 | Consistency (local alarm rules fire) | Consistency (ISA-18.2 compliance) | PC/EC | False negatives are unacceptable [ISA-18.2] |
| Equipment state transitions | L1 | Consistency (state machine continues) | Consistency (sequential ordering) | PC/EC | Invalid transitions corrupt asset model |
| Work order management | L3 | Consistency (session-level) | Consistency | PC/EC | Partial writes create orphaned work orders |
| Dashboard / HMI reads | L2-L3 | Availability (stale data displayed with indicator) | Latency (sub-second preferred) | PA/EL | Stale display is acceptable; blank screen is not |
| Cross-org capacity signals | N/A (Domain 2) | Availability (last-known capacity) | Latency (60s bounded staleness per G-8) | PA/EL | Marketplace tolerates staleness |
| Cross-org marketplace bids | N/A (Domain 2) | Availability (queue locally) | Latency | PA/EL | Bid validity has natural TTL |
| Cross-org bid acceptance | N/A (Domain 2) | Consistency (causal w/ bid) | Consistency | PC/EC | Acceptance MUST follow its bid causally |
| Network aggregates (CRDTs) | N/A (Domain 2) | Availability (converge on heal) | Latency | PA/EL | CRDTs are partition-tolerant by design [CRDT-SHAPIRO] |

**Implementation mapping**:

- **PC/EC paths** (readings, alarms, equipment, work orders): Routed through
  `EventDistribution` (`src/lib/iiot/realtime/event-distribution.ts:280-326`)
  which dual-publishes to local ChannelService (always available) and
  HolonetBridge NATS (best-effort during partition). Local JetStream domain
  preserves ordering guarantees G-1 through G-5 independently of cloud
  connectivity.

- **PA/EL paths** (dashboards, cross-org signals, CRDTs): Served via WebSocket
  subscriptions (`src/lib/iiot/realtime/websocket-server.ts`) with staleness
  metadata. During partition, subscribers receive last-known values with a
  `partitioned: true` indicator. Upon reconnection, fresh data replaces stale
  entries within the G-8 bounded staleness window (60 seconds).

**Key architectural consequence**: The dual-publish path in EventDistribution is
not merely an optimization — it is the mechanism that enables PC/EC for
intra-org data paths. Local ChannelService provides consistency during partition;
HolonetBridge provides cross-node distribution during normal operation.
