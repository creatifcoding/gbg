 ...
 *
 * NOTE: This module uses synchronous Atom.get/set patterns
 * for React integration. The atoms are plain value atoms (not Effect-returning).
 */
...
export const workspaceScopesAtom = Atom.make<Map<WorkspaceId, Record<string, unknown>>>(new Map())

/** Current editor ID */
export const currentEditorAtom = Atom.make<EditorId | null>(null)

/** Editor-specific (buffer-local) configurations */
export const editorScopesAtom = Atom.make<Map<EditorId, Record<string, unknown>>>(new Map())
  ...
  makeProvider({
    load: <A>(def: VariableDef<A>): Effect.Effect<A, VariableError> => {
      ...
      if (typeof defaultVal === 'function') {
        ...
        return Effect.die(
          new Error(`Variable ${def.id} has computed default but no lower scope`)
        )
      }
 ...
 */
export const createUserProvider = (defaults: VariableProvider): VariableProvider => {
  return makeProvider({
    load: <A>(def: VariableDef<A>): Effect.Effect<A, VariableError> => {
      const userScope = Atom.get(userScopeAtom)
      const provider = fromObjectWithDefuFn(userScope, defaults, { name: 'user' })
 ...
 */
export const createWorkspaceProvider = (user: VariableProvider): VariableProvider => {
  return makeProvider({
    load: <A>(def: VariableDef<A>): Effect.Effect<A, VariableError> => {
      const workspaceId = Atom.get(currentWorkspaceAtom)
      if (!workspaceId) {
        // No workspace context — fall through to user
        return user.load(def)
      }

      const workspaceScopes = Atom.get(workspaceScopesAtom)
      const workspaceScope = workspaceScopes.get(workspaceId) ?? {}
      const provider = fromObjectWithDefuFn(workspaceScope, user, { name: 'workspace' })
      return provider.load(def)
    },
    enumerate: () =>
      pipe(
        ...
        Effect.map((userKeys) => {
          const workspaceId = Atom.get(currentWorkspaceAtom)
          const workspaceScopes = Atom.get(workspaceScopesAtom)
          const workspaceScope = workspaceId ? workspaceScopes.get(workspaceId) ?? {} : {}
 ...
 */
export const createEditorProvider = (workspace: VariableProvider): VariableProvider => {
  return makeProvider({
    load: <A>(def: VariableDef<A>): Effect.Effect<A, VariableError> => {
      const editorId = Atom.get(currentEditorAtom)
      if (!editorId) {
        // No editor context — fall through to workspace
        return workspace.load(def)
      }

      const editorScopes = Atom.get(editorScopesAtom)
      const editorScope = editorScopes.get(editorId) ?? {}
      const provider = fromObjectWithDefuFn(editorScope, workspace, { name: 'editor' })
      return provider.load(def)
    },
    enumerate: () =>
      pipe(
...
export const setUserValue = (id: string, value: unknown): void => {
  const current = Atom.get(userScopeAtom)
  Atom.set(userScopeAtom, { ...current, [id]: value })
}
...
export const removeUserValue = (id: string): void => {
  ...
  Atom.set(userScopeAtom, rest)
}

/** Set a workspace-scope value */
export const setWorkspaceValue = (workspaceId: WorkspaceId, id: string, value: unknown): void => {
  const scopes = Atom.get(workspaceScopesAtom)
  ...
  Atom.set(workspaceScopesAtom, newScopes)
}

/** Remove a workspace-scope value */
export const removeWorkspaceValue = (workspaceId: WorkspaceId, id: string): void => {
  const scopes = Atom.get(workspaceScopesAtom)
  const newScopes = new Map(scopes)
  const workspaceScope = newScopes.get(workspaceId)
  if (workspaceScope) {
...
export const setEditorValue = (editorId: EditorId, id: string, value: unknown): void => {
  const scopes = Atom.get(editorScopesAtom)
  const newScopes = new Map(scopes)
  const editorScope = newScopes.get(editorId) ?? {}
  newScopes.set(editorId, { ...editorScope, [id]: value })
  Atom.set(editorScopesAtom, newScopes)
}
...
export const loadWithSource = <A>(
  ...
  Effect.sync(() => {
    const result = resolveValueSync(def.id, def)
