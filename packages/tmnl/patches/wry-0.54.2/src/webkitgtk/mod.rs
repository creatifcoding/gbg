...
impl InnerWebView {
  ...
  fn set_webview_settings(webview: &WebView, attributes: &WebViewAttributes) {
    ...
    if let Some(settings) = WebViewExt::settings(webview) {
      // Work around WebKitGTK accelerated-compositing blank/white windows on
      // Wayland layer-shell surfaces. This must be applied before load_uri/load_html.
      // See: tauri-apps/tauri#15050, webview/webview#1055, wailsapp/wails#2977.
      if std::env::var_os("TMNL_WEBKIT_DISABLE_HARDWARE_ACCELERATION").is_some() {
