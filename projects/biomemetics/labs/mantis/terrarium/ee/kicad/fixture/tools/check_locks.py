...
# Must remain untouched. #24/#25 board trees must stay absent (not recreated).
FORBIDDEN_TOUCH = [
    TERR / "BOM.md",
    TERR / "params.json",
    TERR / "bus.json",
...
def main() -> None:
    ...
    for root in WRITE_ROOTS:
        for path in root.rglob("*"):
            ...
            for line in text.splitlines():
                if '"MPN"' in line or "MPN:" in line or '"mpn"' in line:
    ...
    for rel in (
        ...
        "contracts/ee-interface-v1.json",
    ):
        if not (EE / "kicad" / rel).is_file():
            fail(f"lab-base consume missing: {rel}")

    for board in ABSENT_BOARDS:
        if board.exists():
            fail(f"must not recreate absent board tree: {board.relative_to(EE)}")

    # Write-set only: no files outside the three roots in the staged tree check.
    write_set_prefix = tuple(str(r.resolve()) for r in WRITE_ROOTS)
    for root in WRITE_ROOTS:
        if not root.is_dir():
            fail(f"write-set root missing: {root}")
    ...
    for root in WRITE_ROOTS:
        for path in sorted(root.rglob("*")):
            if path.is_file() and path.name != "manifest.sha256":
                # Guard: every hashed file must live under a write-set root.
                resolved = str(path.resolve())
                if not resolved.startswith(write_set_prefix):
                    fail(f"file outside write set: {path}")
                files.append(path)
    lines = []
    for path in files:
