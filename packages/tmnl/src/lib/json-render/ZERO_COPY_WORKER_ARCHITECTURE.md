# Zero-Copy Worker Architecture for JSON-Render

> Technical analysis of achieving true zero-copy data transfer between main thread and web workers for the JSON-Render streaming UI system.

## Problem Statement

The JSON-Render system streams UI patches from an API, applies them to build a UITree, and renders via React. Currently, all worker communication uses **structured clone**, which copies data on every `postMessage`.
...
For JSON-Render's typical use case (streaming UI, moderate sizes), **MessagePack is the pragmatic choice**.
...
4. **A/B test in JSONRenderTestbed** - measure real-world impact
