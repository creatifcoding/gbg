defmodule Maiden.EquipmentStateRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for EquipmentState transitions.

  Design:
  - Delegates transition execution semantics to `Jido.Agent.Strategy.FSM`.
  - Adds boundary directives after successful transition execution.
  - Keeps strategy execution pure by emitting directives instead of running side effects.

  Boundary directives target:
  - Ash/Ecto persistence (`PersistTransition`)
  - Oban deferred queue (`EnqueueTransitionJob`)
  """
  ...
  def signal_routes(_ctx) do
    [
      {"equipment_state.runtime.strategy.tick", {:strategy_tick}, 90},
      {"equipment_state.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           equipment_state_id: fetch(params, :equipment_state_id) || agent.state[:equipment_state_id],
           machine_id: fetch(params, :machine_id) || agent.state[:machine_id],
           from: from,
           to: to,
           at: at,
           reason: fetch(params, :reason),
           operator_id: fetch(params, :operator_id),
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :equipment_state_runtime_transition)]
          }
        ]
    else
      directives
