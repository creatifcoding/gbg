defmodule Maiden.LineRuntime.Agent do
  @moduledoc """
  Jido-facing line runtime agent.

  ISA-95 alignment:
  - Line is the production/work-center lane under plant/site hierarchy.
  """

  alias Maiden.LineRuntime.Actions.{ObserveRejectedTransition, SetLineStatus}
  alias Maiden.LineRuntime.Boundaries
  alias Maiden.LineRuntime.Directives.{EnqueueTransitionJob, PersistTransition}
  ...
  use Jido.Agent,
    name: "line_runtime_agent",
    description: "Line lifecycle runtime agent with signal-first strategy",
    schema: [
      line_id: [type: :string],
      name: [type: :string],
      status: [type: :string],
      ...
      capacity: [type: :any, default: nil],
      line_type: [type: :any, default: nil],
      operating_hours_per_day: [type: :any, default: nil],
      created_at: [type: :string],
      updated_at: [type: :any, default: nil]
    ],
    signal_routes: [
      {"line.transition.idle", SetLineStatus},
      {"line.transition.running", SetLineStatus},
      {"line.transition.changeover", SetLineStatus},
      {"line.transition.starved", SetLineStatus},
      {"line.transition.blocked", SetLineStatus},
      {"line.transition.maintenance", SetLineStatus},
      {"line.transition.decommissioned", SetLineStatus},
      {"line.transition.rejected", ObserveRejectedTransition}
    ],
    strategy:
      {SignalFsm,
       # Strategy FSM models instruction execution flow; canonical domain
       # transitions are enforced by Maiden.LineRuntime.FSM.
       initial_state: "idle",
       transitions: %{
         "idle" => ["processing"],
       ...
       bridge_jobs?: true,
       job_queue: :line_runtime_transition}

  @transition_actions [SetLineStatus]
  ...
  def preflight_agent_state(payload, opts \\ []),
    do: LineValidator.agent_state_validate(payload, opts)
  ...
  defp normalize_transition_payload(payload) when is_map(payload) do
    %{}
    |> put_if_present(:line_id, payload)
  ...
  defp normalize_state_key("capacity"), do: :capacity
  defp normalize_state_key("line_type"), do: :line_type
  defp normalize_state_key("operating_hours_per_day"), do: :operating_hours_per_day
