...
import {
  ...
  decodeSensorAgentStateSync,
  decodeSensorSync,
  decodeSensorTransitionEventSync,
  isLegalTransition,
  makeSensor,
  makeSensorId,
  makeSensorTransitionEvent,
  toMermaid,
} from './sensor.contract';

describe('sensor.contract', () => {
  it('builds sensor ids using canonical SNS-{slug} format', () => {
    const id = makeSensorId('Main Spindle Temp Probe');
    expect(id).toBe('SNS-main-spindle-temp-probe');
  });

  it('decodes canonical sensor payload', () => {
    const decoded = decodeSensorSync({
      sensor_id: makeSensorId('temp-01'),
  ...
  it('constructs sensor payloads through helper factory', () => {
  ...
  it('constructs transition events through helper factory', () => {
    const event = makeSensorTransitionEvent({
      slug: 'vib-motor-a',
  ...
  it('rejects malformed sensor payloads', () => {
    expect(() =>
      decodeSensorSync({
  ...
  it('schema exports are defined', () => {
    ...
    expect(SensorTransitionEventSchema).toBeDefined();
  });
});
