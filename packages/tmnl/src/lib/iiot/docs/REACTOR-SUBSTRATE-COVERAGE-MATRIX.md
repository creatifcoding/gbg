...
Rolling generated companion: [`REACTOR-TOPOLOGY-ATLAS.md`](./REACTOR-TOPOLOGY-ATLAS.md), generated by `bun run scripts/reactor-topology-atlas.ts`, enumerates the current 101 EventGroup tags, 10 relationship edge types, 87 allowed source/target pairs, and the live production/candidate Reactor lanes. The visual HTML artifact is written to `~/.agent/diagrams/reactor-topology-atlas.html`.
