defmodule Maiden.WorkcellRuntime.Sensors.TransitionSensor do
  @moduledoc """
  Sensor ingress adapter for workcell transition events.
  ...
  use Jido.Sensor,
    name: "workcell_transition_sensor",
    description: "Converts workcell transition events into Jido transition signals",
    schema: Zoi.object(
      %{
        source: Zoi.string() |> Zoi.default("/sensor/workcell-transition"),
  ...
  @impl true
  def handle_event({:workcell_transition, payload}, state) when is_map(payload) do
  ...
  def handle_event(payload, state) when is_map(payload) do
    handle_event({:workcell_transition, payload}, state)
  end

  def handle_event(_event, state), do: {:ok, state}

  defp signal_type(payload), do: "workcell.transition." <> payload["to"]

  defp normalize_payload(payload) do
    %{}
    |> put_if_present(payload, "workcell_id", :workcell_id)
  ...
  defp to_action_params(payload) do
    %{}
    |> put_action_if_present(payload, :workcell_id)
  ...
  defp rejection_payload(payload, reason, trace_id) do
    %{
      workcell_id: Map.get(payload, "workcell_id"),
  ...
  defp attempted_signal_type(payload) do
    case Map.get(payload, "to") do
      to when is_binary(to) and to != "" -> "workcell.transition." <> to
      _ -> "workcell.transition.unknown"
    end
  end
