...
import type { SpecimenCreated, SpecimenEvent } from '../schemas/events'
import type {
  AttachmentId,
  ObservationId,
  QuestionId,
...
import { EventId } from '../schemas/identifiers'
...
function eventId(now: number, suffix: string): SpecimenCreated['id'] {
  return Schema.decodeUnknownSync(EventId)(`evt_${now}_${suffix}`)
}
