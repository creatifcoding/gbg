defmodule Maiden.OrderRuntime.OrderId do
  alias Maiden.OrderRuntime.Support.UUIDv4
  @moduledoc """
  Canonical Order ID builder for Elixir runtime.
  ...
  def normalize_uuid(uuid) when is_binary(uuid) do
    ...
    case UUIDv4.cast(normalized) do
      ...
      :error -> raise ArgumentError, "invalid UUIDv4 for OrderId: #{inspect(uuid)}"
    end
  end
  ...
  defp ensure_valid_slug(slug) do
    if Regex.match?(@slug_regex, slug) do
      ...
      raise ArgumentError, "invalid slug for OrderId: #{inspect(slug)}"
    end
  end
end
