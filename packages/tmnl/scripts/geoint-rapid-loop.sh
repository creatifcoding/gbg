...
echo "[geoint:rapid] 1/3 migrate registry"
bun run geoint:migrate

echo "[geoint:rapid] 2/4 diff seeded registry vs DB"
bun run geoint:registry:diff --strict

echo "[geoint:rapid] 3/4 run registry + planner + parity tests"
bunx vitest run \
...
if [[ "$WITH_INGEST" == "1" ]]; then
  echo "[geoint:rapid] 4/4 run live ingest smoke (RUN_INTEGRATION_TESTS=1)"
  RUN_INTEGRATION_TESTS=1 bunx vitest run src/lib/geoint/ingestion/__tests__/IngestionPipeline.e2e.test.ts
else
  echo "[geoint:rapid] 4/4 ingest smoke skipped (set RUN_GEOINT_INGEST_SMOKE=1 or pass --with-ingest)"
fi
