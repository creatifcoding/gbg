...
import { RlmStore, RlmStoreLive } from "./service.ts"
import { SearchIndex, SearchIndexLive } from "./search.ts"
...
interface CollectionHandle {
  put:     (key: string, data: Record<string, unknown>, tags?: string[]) => Effect.Effect<{ ns: string; key: string }>
  capture: (prefix: string, data: Record<string, unknown>, tags?: string[]) => Effect.Effect<{ ns: string; key: string }>
  get:     (key: string) => Effect.Effect<unknown | null>
  raw:     (key: string) => Effect.Effect<unknown | null>
  meta:    (key: string) => Effect.Effect<Record<string, unknown> | null>
  keys:    () => Effect.Effect<readonly string[]>
  search:  (text: string) => Effect.Effect<readonly any[]>
  count:   () => Effect.Effect<number>
  clear:   () => Effect.Effect<number>
}
...
interface DomainHandle {
  ...
  search: (text: string) => Effect.Effect<readonly any[]>
  events: () => Effect.Effect<readonly FactoryEvent[]>
  subs:   () => string[]
}
...
interface PipelineStageHandle {
  emit:   (data: Record<string, unknown>) => Effect.Effect<{ ns: string; key: string }>
  input:  () => Effect.Effect<unknown | null>
  latest: (n?: number) => Effect.Effect<readonly any[]>
}

interface PipelineRunHandle {
  id:     string
  stage:  (name: string) => PipelineStageHandle
  events: () => Effect.Effect<readonly typeof StageEmit.Type[]>
  state:  () => Effect.Effect<RunState>
}

interface PipelineDefHandle {
  start:  (runId?: string) => Effect.Effect<PipelineRunHandle>
  runs:   () => Effect.Effect<readonly any[]>
  search: (text: string) => Effect.Effect<readonly any[]>
}

interface PipelineFactoryShape {
  define: (name: string, stageNames: readonly string[]) => Effect.Effect<PipelineDefHandle>
}
...
export function createFactoryApi(sqlLayer: Layer.Layer<SqlClient>): FactoryApi {
  ...
  const AppLayer = ServiceLayers.pipe(
    Layer.provide(CollectionFactoryLive),
