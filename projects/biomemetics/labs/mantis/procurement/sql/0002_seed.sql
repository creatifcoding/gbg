...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B08$bom$, $bom$external polymer hinge set$bom$, $bom$2$bom$, $bom$REF$bom$, $bom$CAD miss until a non-JS hinge page is opened. EE candidates none selected: McMaster 1588A714 / 1588A724 / 1588A733. 11565A11 is not a 3 mm acrylic fit claim. Design still REF.$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B16$bom$, $bom$drain plug plus insect baffle$bom$, $bom$1$bom$, $bom$REF$bom$, $bom$CAD miss. UNVERIFIED. No catalog MPN this pass.$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B19$bom$, $bom$P01-P08 continuous electrode flex plus V-dock pads$bom$, $bom$route set$bom$, $bom$UNVERIFIED$bom$, $bom$UNVERIFIED stack-up. CAD miss this pass. No catalog MPN.$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B25$bom$, $bom$normally locked carriage spring$bom$, $bom$1/carriage$bom$, $bom$UNVERIFIED$bom$, $bom$UNVERIFIED rate/PN. CAD miss remains. EE candidate Lee Spring LC032C08M music wire compression spring. Printed rate 3.85 N/mm, free 19.05 mm, solid 8.36 mm, solid load 44.48 N. Arithmetic vs 15-25 N / 5 mm TARGET is ours, not a vendor claim. Pocket UNVERIFIED. Not a buy. Source: https://www.leespring.com/product/compression-spring-lc032c08m-music-wire$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B37$bom$, $bom$internal camera FPC, exact assembly$bom$, $bom$1$bom$, $bom$UNVERIFIED$bom$, $bom$UNVERIFIED length/orientation. CAD miss. No catalog FPC MPN this pass.$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
INSERT INTO part (balloon_id, name, qty_text, class, notes) VALUES ($bom$B39$bom$, $bom$optional captured alignment magnets$bom$, $bom$set$bom$, $bom$UNVERIFIED$bom$, $bom$UNVERIFIED; never primary latch. Candidates none selected: McMaster 3506K21 / 3506K36 / 3506K35 / 5679K88 / 5679K89 / 5679K91.$bom$) ON CONFLICT (balloon_id) DO NOTHING;
...
VALUES (
  ...
  $bom$Bel Fuse$bom$,
  NULL,
  $bom$1 A 0685T1000-01, 2.5 A 0685T2500-01, 3 A 0685T3000-01 are not a 2 A rail fuse. Do not pick a neighbor. Cached prices are not a quote.$bom$
)
ON CONFLICT (id) DO NOTHING;
