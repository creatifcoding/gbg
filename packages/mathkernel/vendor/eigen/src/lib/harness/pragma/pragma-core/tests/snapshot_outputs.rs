...
fn bertscore_known_vectors_snapshot() {
    ...
    let hyp_embs = vec![
        vec![0.9, 0.1, 0.0, 0.0], vec![0.1, 0.9, 0.0, 0.0], vec![0.0, 0.0, 0.8, 0.2],
    ];
    let result = bertscore::compute_from_embeddings(&ref_embs, &hyp_embs);
    insta::assert_json_snapshot!("bertscore_3x3_known", serde_json::json!({
...
fn drift_report_no_drift_snapshot() {
    let embs: Vec<Embedding> = (0..5)
        .map(|i| { let mut v = vec![0.0f32; 8]; v[i % 8] = 1.0; emb(v) })
        .collect();
    let report = drift::detect(&embs, &embs, &drift::DriftConfig::default());
    insta::assert_json_snapshot!("drift_no_drift", serde_json::json!({
...
fn drift_report_severe_snapshot() {
    ...
    let current: Vec<Embedding> = (0..5)
        .map(|i| { let mut v = vec![0.0f32; 8]; v[(i + 4) % 8] = 1.0; emb(v) })
        .collect();
    let report = drift::detect(&baseline, &current, &drift::DriftConfig::default());
    insta::assert_json_snapshot!("drift_severe", serde_json::json!({
        "mean_similarity": (report.mean_similarity * 10000.0).round() / 10000.0,
        "min_similarity": (report.min_similarity * 10000.0).round() / 10000.0,
        "drift_detected": report.drift_detected,
        "severity": format!("{:?}", report.severity),
...
fn catalog_ranking_snapshot() {
    let catalog = CatalogEmbeddings::from_entries(vec![
        CatalogEntry { name: "BarChart".into(), description: "bar chart".into(), embedding: emb(vec![0.9, 0.1, 0.0, 0.0]) },
        CatalogEntry { name: "LoginForm".into(), description: "login form".into(), embedding: emb(vec![0.0, 0.0, 1.0, 0.0]) },
        CatalogEntry { name: "Dashboard".into(), description: "dashboard".into(), embedding: emb(vec![0.7, 0.3, 0.0, 0.0]) },
    ]);
