...
import { createSpecimen } from './entity/specimen'
import { createEdge } from './entity/edge'
...
import { dayStampFromUnix, nextFiledSpecimenId } from './specimen-id'
...
export function fileSpecimen(input: unknown, now = Date.now()): IntakeResult {
  ...
  try {
    ...
    throw new IntakeDecodeError(issuesFromUnknown(error))
  }
...
export class IntakeDecodeError extends Error {
  readonly _tag = 'IntakeDecodeError'
  constructor(readonly issues: ReadonlyArray<string>) {
    super(issues.join('; '))
    this.name = 'IntakeDecodeError'
  }
}
