defmodule Maiden.OrderRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for Order transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"order.runtime.strategy.tick", {:strategy_tick}, 90},
      {"order.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           order_id: fetch(params, :order_id) || agent.state[:order_id],
           from: from,
           to: to,
           at: at,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :order_runtime_transition)]
          }
        ]
    else
      directives
