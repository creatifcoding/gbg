...
fn maybe_init_tmnl_layer_shell(window: &Window, label: &str) {
  let Ok(labels) = std::env::var("TMNL_TAURI_LAYER_SHELL_LABELS") else {
    return;
  };
  ...
  if let Some(screen) = gtk::prelude::WidgetExt::screen(gtk_window) {
    if let Some(visual) = screen.rgba_visual() {
      gtk::prelude::WidgetExt::set_visual(gtk_window, Some(&visual));
    }
  }

  gtk_window.init_layer_shell();

  let namespace = std::env::var("TMNL_TAURI_LAYER_SHELL_NAMESPACE")
    .unwrap_or_else(|_| label.to_string());
  gtk_window.set_namespace(&namespace);

  let layer = match std::env::var("TMNL_TAURI_LAYER_SHELL_LAYER")
    .unwrap_or_else(|_| "overlay".into())
    .to_ascii_lowercase()
    .as_str()
  {
  ...
  let edge = std::env::var("TMNL_TAURI_LAYER_SHELL_EDGE")
  ...
  let size = std::env::var("TMNL_TAURI_LAYER_SHELL_SIZE")
    .ok()
    .and_then(|value| value.parse::<i32>().ok())
    .unwrap_or(48);
  let height = std::env::var("TMNL_TAURI_LAYER_SHELL_HEIGHT")
    .ok()
    .and_then(|value| value.parse::<i32>().ok())
    .unwrap_or(-1);
  ...
  match edge.as_str() {
    ...
    "right" => {
      ...
      gtk_window.set_anchor(gtk_layer_shell::Edge::Top, true);
      gtk_window.set_anchor(gtk_layer_shell::Edge::Bottom, true);
      gtk_window.set_size_request(size, height);
    }
    "top" => {
      gtk_window.set_anchor(gtk_layer_shell::Edge::Top, true);
      gtk_window.set_anchor(gtk_layer_shell::Edge::Left, true);
      gtk_window.set_anchor(gtk_layer_shell::Edge::Right, true);
      gtk_window.set_size_request(-1, size);
    }
    "bottom" => {
      gtk_window.set_anchor(gtk_layer_shell::Edge::Bottom, true);
      gtk_window.set_anchor(gtk_layer_shell::Edge::Left, true);
  ...
  let keyboard = match std::env::var("TMNL_TAURI_LAYER_SHELL_KEYBOARD")
    .unwrap_or_else(|_| "none".into())
    .to_ascii_lowercase()
    .as_str()
  {
    "exclusive" => gtk_layer_shell::KeyboardMode::Exclusive,
    "on-demand" | "ondemand" => gtk_layer_shell::KeyboardMode::OnDemand,
    _ => gtk_layer_shell::KeyboardMode::None,
  };
  gtk_window.set_keyboard_mode(keyboard);

  let exclusive_zone = std::env::var("TMNL_TAURI_LAYER_SHELL_EXCLUSIVE_ZONE")
    .ok()
    .and_then(|value| value.parse::<i32>().ok())
    .unwrap_or(size);
  ...
  log::info!(
    "TMNL layer-shell prebuild init: label={} namespace={} layer={:?} edge={} size={} height={} exclusive_zone={} keyboard={:?}",
    label,
    namespace,
    layer,
    edge,
...
fn maybe_init_tmnl_layer_shell(_window: &Window, _label: &str) {}
...
fn create_window<T: UserEvent, F: Fn(RawWindow) + Send + 'static>(
  ...
  // TMNL/GetByShell: the Wayland layer-shell role must be assigned to the
  // GTK toplevel before Wry constructs WebKitGTK into default_vbox(). An
  // app-level reparent of an already-realized WebKit subtree keeps DOM/IPC
  // alive but can leave the layer-shell surface black under DriftWM.
  maybe_init_tmnl_layer_shell(&window, &label);
...
"#,
  ...
  let webview = match kind {
    ...
    WebviewKind::WindowContent => {
      ...
      let builder = webview_builder.build_gtk(window.default_vbox().unwrap());
      builder
    }
  }
