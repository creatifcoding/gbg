/**
 * DurableStreamWire — the public, transport-agnostic wire interface.
 *
 * `DurableStreamWire` is a `Context.Service` whose shape is **hand-curated**
 * (deliberately not auto-derived from `RpcClient.From<typeof Wire>`, which
 ...
 *
 * Each implementation is a `Layer<DurableStreamWire, ..., InternalService>`
 * that composes a per-impl internal Context.Service (`InMemoryInner`,
 * `HttpInner`, `NatsBridgeInner`).
 *
 * # Drift guard
 *
 * The `_DriftGuard` type below ensures `DurableStreamWireShape`'s method tags
 * remain a subset of the `Wire` RpcGroup's Rpc tags. Adding a new Rpc to
 * `Wire` without adding a corresponding method here will fail to compile.
 *
 * @module @tmnl/lnk/services/wire/DurableStreamWire
 */
...
// ─── Type extraction from the Wire spec ─────────────────────────────────────
...
/**
 * The methods exposed by `DurableStreamWire`.
 *
 ...
 * source of truth (the `Wire` RpcGroup) while letting us hand-pick the
 * runtime shape (no `AsQueue` / `Discard` flags).
 */
export interface DurableStreamWireShape {
...
type _WireTags = RpcGroup.Rpcs<typeof Wire>["_tag"]
type _CuratedTags = keyof DurableStreamWireShape

type _DriftGuard = [_CuratedTags] extends [_WireTags]
  ? [_WireTags] extends [_CuratedTags]
    ? true
    : "FAIL: Wire has Rpcs not present in DurableStreamWireShape"
  : "FAIL: DurableStreamWireShape has methods not present in Wire"
...
/**
 * The wire service. Use `yield* DurableStreamWire` in `Effect.gen` to access
 ...
 * ```
 */
export class DurableStreamWire extends Context.Service<
  DurableStreamWire,
  DurableStreamWireShape
>()("@tmnl/lnk/services/wire/DurableStreamWire") {}
