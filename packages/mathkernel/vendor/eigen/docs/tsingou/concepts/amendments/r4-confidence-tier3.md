...
### R4.3: Dempster-Shafer vs Weighted Average — When to Use Which

**Problem**: The academic data fusion literature heavily references Dempster-Shafer
(D-S) evidence theory as the principled way to combine evidence under uncertainty.
TSGC-001 uses weighted averages. Which is correct?

**Grounded theory finding**: D-S is widely cited but rarely deployed as the primary
fusion confidence model in production intelligence/security systems.
[TSGC-GT-001 Section 2.3, 2.8]

**Comparative analysis**:

```
+---------------------------+----------------------------+------------------------------+
|                           | WEIGHTED AVERAGE           | DEMPSTER-SHAFER              |
+---------------------------+----------------------------+------------------------------+
| Math complexity           | Simple arithmetic          | Mass functions, Dempster's   |
|                           |                            | combination rule, belief/     |
|                           |                            | plausibility intervals        |
+---------------------------+----------------------------+------------------------------+
| Operator interpretability | HIGH — "0.72 means the     | LOW — "belief 0.6, plaus.    |
|                           | weighted evidence is 72%"  | 0.85" requires training       |
+---------------------------+----------------------------+------------------------------+
| Conflicting evidence      | Averages it out (can mask  | Redistributes mass, but      |
|                           | conflict)                  | Zadeh's paradox: highly      |
|                           |                            | conflicting sources produce  |
|                           |                            | counterintuitive results     |
+---------------------------+----------------------------+------------------------------+
| Correlated evidence       | Needs explicit discount    | Assumes independence;        |
|                           | (R4.2)                     | correlated sources violate   |
|                           |                            | combination rule assumptions |
+---------------------------+----------------------------+------------------------------+
| Partial ignorance         | Cannot represent "I don't  | CAN represent ignorance via  |
|                           | know" — must assign score  | uncommitted mass             |
+---------------------------+----------------------------+------------------------------+
| Calibration               | Ordinal by default,        | Inherently uncalibrated;     |
|                           | calibratable (R4.4)        | belief/plausibility are not  |
|                           |                            | probabilities                |
+---------------------------+----------------------------+------------------------------+
| Production deployments    | Universal — every surveyed | Rare — fault diagnosis, some |
|                           | system uses some form      | sensor reliability contexts  |
+---------------------------+----------------------------+------------------------------+
| Failure mode              | Predictable — degrades     | Unpredictable — Zadeh's      |
|                           | gracefully                 | paradox can produce absurd   |
|                           |                            | results                      |
+---------------------------+----------------------------+------------------------------+
```

**Recommendation**: Weighted average as PRIMARY, with D-S available as an OPTIONAL
module for specific use cases.

```
USE WEIGHTED AVERAGE (default) when:
  - Operator interpretability is required (always, for Tsingou)
  - Evidence sources are generally consistent
  - Confidence needs to be a single number for threshold comparison
  - Calibration feedback loop is planned (R4.4)

USE DEMPSTER-SHAFER when (optional module, operator-activated):
  - Representing explicit ignorance matters ("we have NO evidence for
    or against this hypothesis")
  - Source reliability assessment (meta-question: "how much should I
    trust sensor X?")
  - The specific use case has been validated with D-S on representative
    data before production deployment

NEVER USE DEMPSTER-SHAFER when:
  - Sources are known to be highly conflicting (Zadeh's paradox risk)
  - Sources are correlated (violates combination rule assumptions)
  - Operators cannot be trained on belief/plausibility interpretation
```

**Schema addition**:

```typescript
// Add to FusionOntology thresholds
confidenceModel: Schema.Literal(
  "weighted_average",    // Default — simple, interpretable, production-proven
  "log_odds",            // Alternative — better extreme value handling (R4.4)
  "dempster_shafer"      // Expert — requires operator training, use sparingly
),
```

**D-S module specification** (when enabled):

```
DEMPSTER-SHAFER MODULE (optional, expert mode):

  Representation:
    Each predicate produces a mass function m(H) over hypotheses:
      H = { match, no_match, unknown }
      m(match) + m(no_match) + m(unknown) = 1.0

  Combination:
    For N independent evidence sources:
      m_combined = m_1 ⊕ m_2 ⊕ ... ⊕ m_N  (Dempster's rule)

  Conflict detection:
    K = SUM over conflicting focal elements
    If K > 0.7: WARN operator — "highly conflicting evidence"
    If K > 0.9: REFUSE to combine — present evidence separately

  Output:
    belief(match) ≤ probability ≤ plausibility(match)
    Display as interval: [Bel, Pl] instead of point estimate
    Operator sees: "Confidence: [0.55, 0.82]" meaning
      "at least 55% sure, at most 82% sure"

  Conversion to point estimate (for threshold comparison):
    C_point = (belief + plausibility) / 2  // midpoint of interval
    // OR: C_point = belief + (plausibility - belief) * optimism_factor
    //   where optimism_factor ∈ [0, 1] is operator-configurable
```

---

### R4.4: Log-Odds for Extreme Values (formerly R4.3)
...
### R4.5: Confidence Calibration Framework
...
### R4.6: Bayesian Confidence Refinement on Signal Arrival
...
### R4.7: Updated Section 9 — Confidence Semantics Table
...
| R4.3 | 3.2.2 | D-S vs weighted average: when to use which | MEDIUM |
| R4.4 | 3.2.2 | Optional log-odds combination mode | LOW |
| R4.5 | 10 (RI-4) | Confidence calibration framework (4 phases) | MEDIUM |
| R4.6 | 3.2.2 | Bayesian confidence refinement on signal arrival | MEDIUM |
| R4.7 | 9 | Updated confidence semantics table | MEDIUM |
