...
ELIXIR_DIR="$PROJECT_ROOT/src/lib/maidens/domains/contracts/area/elixir"
SCHEMA_DIR="$PROJECT_ROOT/src/lib/maidens/domains/contracts/area/schemas"
REPORT_DIR="$PROJECT_ROOT/src/lib/maidens/domains/contracts/area/reports"
RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)"
REPORT_PATH="${AREA_E2E_REPORT_PATH:-$REPORT_DIR/area-e2e-${RUN_ID}.json}"
LATEST_REPORT_PATH="$REPORT_DIR/latest.json"
LOG_DIR="$REPORT_DIR/logs/$RUN_ID"
STEPS_JSONL="$REPORT_DIR/area-e2e-${RUN_ID}.steps.jsonl"
...
schema_fingerprint() {
  find "$SCHEMA_DIR" -maxdepth 1 -type f -name 'area*' -print0 \
    | sort -z \
    | xargs -0 sha256sum \
    | sha256sum \
    | awk '{print $1}'
...
run_schema_determinism_step() {
  ...
  local command="bun src/lib/maidens/domains/contracts/area/scripts/gen-area-schemas.ts"
  ...
  echo "[area-e2e] first generation run" | tee -a "$log_path"
  (cd "$cwd" && bash -lc "$command") 2>&1 | tee -a "$log_path"
  exit_code=$?

  if [[ "$exit_code" -eq 0 ]]; then
    fingerprint_before="$(schema_fingerprint)"
    echo "[area-e2e] fingerprint before rerun: $fingerprint_before" | tee -a "$log_path"

    echo "[area-e2e] second generation run" | tee -a "$log_path"
    (cd "$cwd" && bash -lc "$command") 2>&1 | tee -a "$log_path"
    exit_code=$?

    if [[ "$exit_code" -eq 0 ]]; then
      fingerprint_after="$(schema_fingerprint)"
      echo "[area-e2e] fingerprint after rerun:  $fingerprint_after" | tee -a "$log_path"

      if [[ "$fingerprint_before" != "$fingerprint_after" ]]; then
        echo "[area-e2e] ERROR: schema generation is non-deterministic" | tee -a "$log_path"
        echo "before: $fingerprint_before" | tee -a "$log_path"
        echo "after : $fingerprint_after" | tee -a "$log_path"
        exit_code=42
      else
        echo "[area-e2e] schema fingerprint stable: $fingerprint_after" | tee -a "$log_path"
      fi
    fi
  fi
...
printf '\n[area-e2e] project root: %s\n' "$PROJECT_ROOT"

overall_status=0

if run_step "ts_contract_tests" "TypeScript contract tests" "1/6" "$PROJECT_ROOT" "bunx vitest run src/lib/maidens/domains/contracts/area/ts/area.contract.test.ts"; then
  if run_schema_determinism_step; then
    if run_step "elixir_runtime_tests" "Elixir runtime tests (validator + strategy + persistence)" "3/6" "$ELIXIR_DIR" "mix deps.get >/dev/null && mix test"; then
      if run_step "strategy_boundary_tests" "strategy boundary tests" "4/6" "$ELIXIR_DIR" "mix test test/area_strategy_boundary_test.exs"; then
        if run_step "elixir_persistence_tests" "explicit persistence test file" "5/6" "$ELIXIR_DIR" "mix test test/area_persistence_test.exs"; then
          if ! run_step "negative_gate_assertions" "negative-gate assertions (schema-valid/FSM-illegal rejection)" "6/6" "$ELIXIR_DIR" "mix test test/area_validator_test.exs --only negative_gate"; then
            overall_status=1
          fi
        else
          overall_status=1
          append_skipped_step "negative_gate_assertions" "negative-gate assertions (schema-valid/FSM-illegal rejection)" "mix test test/area_validator_test.exs --only negative_gate" "$ELIXIR_DIR"
        fi
      else
        overall_status=1
        append_skipped_step "elixir_persistence_tests" "explicit persistence test file" "mix test test/area_persistence_test.exs" "$ELIXIR_DIR"
        append_skipped_step "negative_gate_assertions" "negative-gate assertions (schema-valid/FSM-illegal rejection)" "mix test test/area_validator_test.exs --only negative_gate" "$ELIXIR_DIR"
      fi
    else
      overall_status=1
      append_skipped_step "strategy_boundary_tests" "strategy boundary tests" "mix test test/area_strategy_boundary_test.exs" "$ELIXIR_DIR"
      append_skipped_step "elixir_persistence_tests" "explicit persistence test file" "mix test test/area_persistence_test.exs" "$ELIXIR_DIR"
      append_skipped_step "negative_gate_assertions" "negative-gate assertions (schema-valid/FSM-illegal rejection)" "mix test test/area_validator_test.exs --only negative_gate" "$ELIXIR_DIR"
    fi
  else
  ...
  overall_status=1
  append_skipped_step "schema_generation_determinism" "JSON Schema generation + determinism" "bun src/lib/maidens/domains/contracts/area/scripts/gen-area-schemas.ts" "$PROJECT_ROOT"
  append_skipped_step "elixir_runtime_tests" "Elixir runtime tests (validator + strategy + persistence)" "mix deps.get >/dev/null && mix test" "$ELIXIR_DIR"
  append_skipped_step "strategy_boundary_tests" "strategy boundary tests" "mix test test/area_strategy_boundary_test.exs" "$ELIXIR_DIR"
  append_skipped_step "elixir_persistence_tests" "explicit persistence test file" "mix test test/area_persistence_test.exs" "$ELIXIR_DIR"
  append_skipped_step "negative_gate_assertions" "negative-gate assertions (schema-valid/FSM-illegal rejection)" "mix test test/area_validator_test.exs --only negative_gate" "$ELIXIR_DIR"
...
if [[ "$overall_status" -eq 0 ]]; then
  ci_status="passed"
  printf '\n[area-e2e] ✅ all gates passed\n'
  printf '[area-e2e] report: %s\n' "$report_output_path"
else
  printf '\n[area-e2e] ❌ one or more gates failed\n'
  printf '[area-e2e] report: %s\n' "$report_output_path"
fi

summary_line="status=${ci_status} report=${report_output_path}"
printf '[area-e2e][ci-summary] %s\n' "$summary_line"
printf '::notice title=area-e2e::%s\n' "$summary_line"
