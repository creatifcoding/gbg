defmodule Maiden.DeviceRuntime.FSM do
  @moduledoc """
  Explicit device transition legality layer.

  Mirrors `src/lib/maidens/domains/contracts/device/ts/device.contract.ts`.
  """
  ...
  def validate_transition_for_runtime(payload, opts \\ []) when is_map(payload) do
    ...
    with :ok <- DeviceValidator.transition_event_validate(payload, opts),
  ...
  def validate_transition_for_jido(payload, opts \\ []), do: validate_transition_for_runtime(payload, opts)
end
