defmodule Maiden.SensorRuntime.Strategies.SignalFsm do
  @moduledoc """
  Signal-first runtime strategy for sensor transitions.
  ...
  def signal_routes(_ctx) do
    [
      {"sensor.runtime.strategy.tick", {:strategy_tick}, 90},
      {"sensor.runtime.persist.flush", {:strategy_cmd, :flush_persistence}, 85}
    ]
  end
  ...
  defp transition_event(payload, agent) do
    ...
    cond do
      ...
      true ->
        {:ok,
         %{
           sensor_id: fetch(params, :sensor_id) || agent.state[:sensor_id],
           from: from,
           to: to,
           at: at,
      ...
      directives ++
        [
          %EnqueueTransitionJob{
            event: event,
            opts: [queue: Keyword.get(opts, :job_queue, :sensor_runtime_transition)]
          }
        ]
    else
      directives
