...
export const columnLayoutStyles = `
  ...
  .column-layout {
    position: relative;
    margin: ${spacing[4]} 0;
    padding-top: ${spacing[6]}; /* Space for badge */
  }

  .column-layout:hover .column-layout-badge {
    opacity: 1 !important;
  }
  ...
  /* CRITICAL: NodeViewContent creates an intermediate wrapper that breaks grid layout.
   * display: contents makes it "invisible" to CSS box model, so columns become
