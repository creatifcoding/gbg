...
# Repetition {#_top .astro-np5lzwrf}
:::
::::
...
:::::::::::: {.sl-container .astro-2wc2n45z style="--borderTop: initial;--pageTitleDisplay: block;"}
::::::: sl-markdown-content
Repetition is a common requirement when working with effects in software
development. It allows us to perform an effect multiple times according
to a specific repetition policy.
...
aria-hidden="true"}](index.html#repeat){.anchor-link
aria-labelledby="repeat"}
:::

The `Effect.repeat`{dir="auto"} function returns a new effect that
repeats the given effect according to a specified schedule or until the
first failure.
...
.astro-4rgy7crp} Initial Execution Included

::: starlight-aside__content
The scheduled recurrences are in addition to the initial execution, so
`Effect.repeat(action, Schedule.once)`{dir="auto"} executes
`action`{dir="auto"} once initially, and if it succeeds, repeats it an
additional time.
:::
...
::: code
[// Define an effect that logs a message to the
...
[log]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[\"success\"]{style="--0:#032F62;--1:#9ECBFF"}[)]{style="--0:#24292E;--1:#E1E4E8"}
::::::::::
:::::::::::::
...
::: code
[// Define a schedule that repeats the action 2 more times with a
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const addDelay: &lt;number, unknown, never&gt;(self: Schedule.Schedule&lt;number, unknown, never&gt;, f: (out: number) =&gt; DurationInput) =&gt; Schedule.Schedule&lt;number, unknown, never&gt; (+1 overload)</code></pre>
</figure>
:::
...
::: code
[// Repeat the action according to the
...
::: code
[// Run the program and log the number of
repetitions]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
[log]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[\`repetitions:
...
::: code
[repetitions: 2]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>let count: number</code></pre>
</figure>
:::
::::
...
::: code
[// Define an async effect that simulates an action with potential
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const action: Effect.Effect&lt;string, string, never&gt;</code></pre>
</figure>
:::
::::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const async: &lt;string, string, never&gt;(resume: (callback: (_: Effect.Effect&lt;string, string, never&gt;) =&gt; void, signal: AbortSignal) =&gt; void | Effect.Effect&lt;void, never, never&gt;, blockingOn?: FiberId) =&gt; Effect.Effect&lt;string, string, never&gt;</code></pre>
</figure>
:::
...
- Can return an optional `Effect` that will be run if the `Fiber`
  executing this `Effect` is interrupted. This can be useful in
  scenarios where you need to handle resource cleanup if the operation
  is interrupted.
- Can receive an `AbortSignal` to handle interruption if needed.

The `FiberId` of the fiber that may complete the async callback may also
be specified using the `blockingOn` argument. This is called the
\"blocking fiber\" because it suspends the fiber executing the `async`
effect (i.e. semantically blocks the fiber from making progress).
...
Use `Effect.async` when dealing with APIs that use callback-style
instead of `async/await` or `Promise`.

**Example** (Wrapping a Callback API)

::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>import { Effect } from &quot;effect&quot;import * as NodeFS from &quot;node:fs&quot;
...
//      ┌─── Effect&lt;Buffer, Error, never&gt;//      ▼const program = readFile(&quot;example.txt&quot;)</code></pre>
</figure>
:::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>import { Effect, Fiber } from &quot;effect&quot;import * as NodeFS from &quot;node:fs&quot;
...
// Run the programEffect.runPromise(program)// Output:// Cleaning up example.txt</code></pre>
</figure>
:::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>import { Effect, Fiber } from &quot;effect&quot;
...
// Run the programEffect.runPromise(program)// Output:// Abort signal received</code></pre>
</figure>
:::
::::::
...
[async]{style="--0:#6F42C1;--1:#B392F0"}[\<]{style="--0:#24292E;--1:#E1E4E8"}[string]{style="--0:#005CC5;--1:#79B8FF"}[,
]{style="--0:#24292E;--1:#E1E4E8"}[string]{style="--0:#005CC5;--1:#79B8FF"}[\>((]{style="--0:#24292E;--1:#E1E4E8"}[[]{.twoslash-hover}]{.twoslash
style="--0:#AE4B07;--1:#FFAB70"}
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>resume: (_: Effect.Effect&lt;string, string, never&gt;) =&gt; void</code></pre>
</figure>
:::
::::

[resume]{style="--0:#AE4B07;--1:#FFAB70"}[)
]{style="--0:#24292E;--1:#E1E4E8"}[=\>]{style="--0:#BF3441;--1:#F97583"}[
{]{style="--0:#24292E;--1:#E1E4E8"}
:::::::::::::::::
::::::::::::::::::::
...
[log]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[\"failure\"]{style="--0:#032F62;--1:#9ECBFF"}[)]{style="--0:#24292E;--1:#E1E4E8"}
::::::::::::::
:::::::::::::::::
...
[resume]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[[]{.twoslash-hover}]{.twoslash
style="--0:#24292E;--1:#E1E4E8"}
...
[count]{style="--0:#24292E;--1:#E1E4E8"}[++]{style="--0:#BF3441;--1:#F97583"}
:::::
::::::::
...
[succeed]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[\"yay!\"]{style="--0:#032F62;--1:#9ECBFF"}[))]{style="--0:#24292E;--1:#E1E4E8"}
:::::::::::::
::::::::::::::::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const repeat: &lt;string, string, never, number, never&gt;(self: Effect.Effect&lt;string, string, never&gt;, schedule: Schedule.Schedule&lt;number, string, never&gt;) =&gt; Effect.Effect&lt;number, string, never&gt; (+3 overloads)</code></pre>
</figure>
:::
...
::: code
[// Run the program and observe the result on
failure]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const runPromiseExit: &lt;number, string&gt;(effect: Effect.Effect&lt;number, string, never&gt;, options?: {    readonly signal?: AbortSignal;} | undefined) =&gt; Promise&lt;Exit&lt;number, string&gt;&gt;</code></pre>
</figure>
:::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>Promise&lt;Exit&lt;number, string&gt;&gt;.then&lt;void, never&gt;(onfulfilled?: ((value: Exit&lt;number, string&gt;) =&gt; void | PromiseLike&lt;void&gt;) | null | undefined, onrejected?: ((reason: any) =&gt; PromiseLike&lt;never&gt;) | null | undefined): Promise&lt;void&gt;</code></pre>
</figure>
:::
...
[[![](data:image/svg+xml;base64,)]{.anchor-icon
aria-hidden="true"}](index.html#skipping-first-execution){.anchor-link
aria-labelledby="skipping-first-execution"}
:::

If you want to avoid the first execution and only run the action
according to a schedule, you can use `Effect.schedule`{dir="auto"}. This
allows the effect to skip the initial run and follow the defined repeat
policy.
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const schedule: &lt;void, never, never, never, number&gt;(self: Effect.Effect&lt;void, never, never&gt;, schedule: Schedule.Schedule&lt;number, void | undefined, never&gt;) =&gt; Effect.Effect&lt;number, never, never&gt; (+1 overload)</code></pre>
</figure>
:::
...
This function allows you to execute an effect repeatedly according to a
given schedule. The schedule determines the timing and number of
repetitions. Each repetition can also depend on the decision of the
schedule, providing flexibility for complex workflows. This function
does not modify the effect\'s success or failure; it only controls its
repetition.

For example, you can use a schedule that recurs a specific number of
times, adds delays between repetitions, or customizes repetition
behavior based on external inputs. The effect runs initially and is
repeated according to the schedule.
:::

::: {.twoslash-popup-docs .twoslash-popup-docs-tags}
[\@see]{.twoslash-popup-docs-tag-name} ― [scheduleFrom for a variant
that allows the schedule\'s decision to depend on the result of this
...
[[![](data:image/svg+xml;base64,)]{.anchor-icon
aria-hidden="true"}](index.html#repeatn){.anchor-link
aria-labelledby="repeatn"}
:::

The `repeatN`{dir="auto"} function returns a new effect that repeats the
specified effect a given number of times or until the first failure. The
repeats are in addition to the initial execution, so
`Effect.repeatN(action, 1)`{dir="auto"} executes `action`{dir="auto"}
...
::: code
[// Repeat the action 2 additional times after the first
...
[[![](data:image/svg+xml;base64,)]{.anchor-icon
aria-hidden="true"}](index.html#repeatorelse){.anchor-link
aria-labelledby="repeatorelse"}
:::

The `repeatOrElse`{dir="auto"} function returns a new effect that
repeats the specified effect according to the given schedule or until
the first failure. When a failure occurs, the failure value and schedule
output are passed to a specified handler. Scheduled recurrences are in
addition to the initial execution, so
`Effect.repeat(action, Schedule.once)`{dir="auto"} executes
`action`{dir="auto"} once initially and then repeats it an additional
time if it succeeds.
...
::: code
[// Define an async effect that simulates an action with possible
...
::: code
[// Define a schedule that repeats up to 2
...
::: code
[// Provide a handler to run when failure occurs after the
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const repeatOrElse: &lt;string, string, never, never, number, never, never&gt;(self: Effect.Effect&lt;string, string, never&gt;, schedule: Schedule.Schedule&lt;number, string, never&gt;, orElse: (error: string, option: Option&lt;number&gt;) =&gt; Effect.Effect&lt;number, never, never&gt;) =&gt; Effect.Effect&lt;number, never, never&gt; (+1 overload)</code></pre>
</figure>
:::

:::: twoslash-popup-docs
Repeats an effect with a schedule, handling failures using a custom
handler.

**Details**

This function allows you to execute an effect repeatedly based on a
specified schedule. If the effect fails at any point, a custom failure
handler is invoked. The handler is provided with both the failure value
and the output of the schedule at the time of failure. This enables
advanced error recovery or alternative fallback logic while maintaining
flexibility in how repetitions are handled.

For example, using a schedule with `recurs(2)` will allow for two
additional repetitions after the initial execution, provided the effect
succeeds. If a failure occurs during any iteration, the failure handler
is invoked to handle the situation.
...
[repeatOrElse]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[[]{.twoslash-hover}]{.twoslash
style="--0:#24292E;--1:#E1E4E8"}
...
[policy]{style="--0:#24292E;--1:#E1E4E8"}[, ()
]{style="--0:#24292E;--1:#E1E4E8"}[=\>]{style="--0:#BF3441;--1:#F97583"}
:::::::::::::::::
::::::::::::::::::::
...
[log]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[\"orElse\"]{style="--0:#032F62;--1:#9ECBFF"}[)]{style="--0:#24292E;--1:#E1E4E8"}
::::::::::::::
:::::::::::::::::
...
::: code
[orElse]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: code
[repetitions: 1]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
[[![](data:image/svg+xml;base64,)]{.anchor-icon
aria-hidden="true"}](index.html#repeating-based-on-a-condition){.anchor-link
aria-labelledby="repeating-based-on-a-condition"}
:::

You can control the repetition of an effect by a condition using either
a `while`{dir="auto"} or `until`{dir="auto"} option, allowing for
dynamic control based on runtime outcomes.
...
::: code
[// Define an effect that simulates varying outcomes on each
invocation]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const action: Effect.Effect&lt;number, never, never&gt;</code></pre>
</figure>
:::
::::
...
[log]{style="--0:#6F42C1;--1:#B392F0"}[(]{style="--0:#24292E;--1:#E1E4E8"}[\`Action
called
\${]{style="--0:#032F62;--1:#9ECBFF"}[++]{style="--0:#BF3441;--1:#F97583"}[[]{.twoslash-hover}]{.twoslash
style="--0:#24292E;--1:#E1E4E8"}
...
[count]{style="--0:#24292E;--1:#E1E4E8"}[}
time(s)\`]{style="--0:#032F62;--1:#9ECBFF"}[)]{style="--0:#24292E;--1:#E1E4E8"}
::::::::::::::::
:::::::::::::::::::
...
::: code
[// Repeat the action until the count reaches
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const program: Effect.Effect&lt;3, never, never&gt;</code></pre>
</figure>
:::
::::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const repeat: &lt;number, never, never, {    until: (n: number) =&gt; n is 3;}&gt;(self: Effect.Effect&lt;number, never, never&gt;, options: {    until: (n: number) =&gt; n is 3;}) =&gt; Effect.Effect&lt;3, never, never&gt; (+3 overloads)</code></pre>
</figure>
:::
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>until: (n: number) =&gt; n is 3</code></pre>
</figure>
:::
::::

[until]{style="--0:#6F42C1;--1:#B392F0"}[:
(]{style="--0:#24292E;--1:#E1E4E8"}[[]{.twoslash-hover}]{.twoslash
style="--0:#AE4B07;--1:#FFAB70"}
...
::: expressive-code
<figure class="frame">
<pre data-language="ts"><code>const runFork: &lt;3, never&gt;(effect: Effect.Effect&lt;3, never, never&gt;, options?: RunForkOptions) =&gt; RuntimeFiber&lt;3, never&gt;</code></pre>
</figure>
:::
...
::: code
[Action called 1 time(s)]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: code
[Action called 2 time(s)]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
::: code
[Action called 3 time(s)]{style="--0:#616972;--1:#99A0A6"}
:::
::::::
...
.astro-4rgy7crp} Retrying on Errors

::: starlight-aside__content
You can use
[Effect.retry](../../error-management/retrying/index.html#retrying-based-on-a-condition)
...
.astro-4rgy7crp} Edit
page](https://github.com/Effect-TS/website/edit/main/content/src/content/docs/docs/scheduling/repetition.mdx){.sl-flex
